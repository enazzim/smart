<%@ Control Language="c#" AutoEventWireup="false" Codebehind="ItemSearch.ascx.cs" Inherits="KIT_ERP.ProductionManagement.ItemSearch.ItemSearch" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
...
<!--
	function openPopupChild(id, controlName, propertyClassification, businessRegistrationNum, unitcostDistinction)
	{
		...
		arr = window.showModalDialog
			("ItemSearch/popUp_ItemSearch.aspx?tag="+ controlName + "&value=" + value + "&propertyClassification=" + propertyClassification + "&businessregistrationnum=" + businessRegistrationNum + "&unitcostdistinction=" + unitcostDistinction ,"f", "dialogHeight: 400px; dialogWidth: 807px; center: yes; help: no; resizable: no; status: no;");

		if (arr != null) 
		{				
			document.all["<%= ChildtxtItemNum.ClientID %>"].value = arr["ItemNum"];
			document.all["<%= hdChildItemNum.ClientID %>"].value = arr["ItemNum"];
	...
	function ResetBox()
	{
		...
		document.all["<%= hdChildItemNum.ClientID %>"].value = "";
	}
...
<table border="0" cellpadding="0" cellspacing="0" width="600" height="20">
	<tr>
		<td width="70" vAlign="middle" align="right" style="FONT-SIZE: 9pt">
			<asp:Label id="lbChildItemNum" runat="server">품목번호</asp:Label>&nbsp;
		</td>
		...
		<td width="70" vAlign="middle" align="right" style="FONT-SIZE: 9pt">
			<asp:Label id="lbChildItemDrawNum" runat="server">도면번호</asp:Label>&nbsp;
		</td>
		<td width="130">
		...
		<td width="70" vAlign="middle" align="right" style="FONT-SIZE: 9pt">
			<asp:Label id="lbChildItemName" runat="server">품목명</asp:Label>
		</td>
		<td width="130">
...
<INPUT id="hdChildItemNum" style="WIDTH: 32px; HEIGHT: 22px" type="hidden" size="1" name="hdChildItemNum"
	runat="server">
