...
namespace KIT_ERP.QualityInspection
{
	...
	public class OutsiderOrder_Out_Item : KIT_ERP.QualityInspection.QualityInspection_BaseClass
	{
		...
		public DataTable getData()
		{
			...
			cmd.Parameters.Add( "@ItemNum", SqlDbType.VarChar).Value = this.m_itemNum;
			...
			if ( dr.Read() )	// 이전 공정이 있다면(1:1 인경우에 해당됨.) 이전공정품이 전부 출력된다.
			{
				tmpItem_DataRow = dt_Result.NewRow();
				tmpItem_DataRow[0] = dr[0].ToString();
				tmpItem_DataRow[1]= Convert.ToInt32( dr[1].ToString() );
				tmpItem_DataRow[2]= 1;		// 1:1 이므로 1개의 품목이 출고되었음. 호출한 코드에서 여기에 합격수량을 곱해준다.
...
//============================================================================================================
		...
		private DataTable get_ChildItem_ProcessSequenceNum ( string strItemNum, string strNeedQuantity )
		{
			...
			adap.SelectCommand.Parameters.Add("@ItemNum", SqlDbType.VarChar).Value = strItemNum;
		...
		private DataTable get_ChildItem(int level, string ItemNum)
		{
			...
			adap.SelectCommand.Parameters.Add("@ItemNum", SqlDbType.VarChar).Value = ItemNum;
		...
		private void Search(Stack st_ChildItem,  string ItemNum)
		{
			...
			int nCount = st_ChildItem.Count;

			for ( int i = 0 ; i < nCount ; i ++ )			// 스택 1 이 다 종료 될때 까지..
			{
				if(st_ChildItem.Count != 0)
				{
					...
					else		// 반제품이면...
					{
						...
						int nCount2 = st_ProcessSequenceItem.Count;
						for ( int j = 0 ; j < nCount2 ; j ++ )	// 스택 2 가 종료 될때 까지
						{
							// 공정순서 Item 뽑아내기
