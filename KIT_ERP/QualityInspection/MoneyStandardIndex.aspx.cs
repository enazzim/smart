...
{
	/// <summary>
	/// MoneyStandardIndex에 대한 요약 설명입니다.
	/// </summary>
	public class MoneyStandardIndex :  QualityInspection.QualityInspection_BaseClass
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				//Response.Write("<script language='javascript'>document.parentWindow.parent.ContentsTitle.location = '../ContentsTitle.aspx?title=∵ 경영정보 > 금액기준 지표';</script>");
			}
		}
		...
		private void btnSearch_Click(object sender, System.EventArgs e)
		{
			...
			if ( ddlDIV.SelectedIndex > 0 )	strDIV = ddlDIV.SelectedItem.Text;

			if ( ddlItemClassification1.SelectedIndex > 0 )	strItemClassification1 = ddlItemClassification1.SelectedValue;

			if ( ddlItemClassification2.SelectedIndex > 0 )	strItemClassification2 = ddlItemClassification2.SelectedValue;

			if ( ddlItemClassification3.SelectedIndex > 0 )	strItemClassification3 = ddlItemClassification3.SelectedValue;

			if ( ddlItemClassification4.SelectedIndex > 0 )	strItemClassification4 = ddlItemClassification4.SelectedValue;
			
			// WebCombo Display Value
			...
			{
				DataGrid1.DataSource = this.ProcessGridSource( dt_GridSource );
				DataGrid1.DataBind();

				// ChartData 가공
				UltraChart1.Data.DataSource = this.ProcessChartSource( dt_GridSource.Copy() ).DefaultView;
		...
		/// <summary>
		/// 데이터 원본(DataTable)으로부터 Grid에 표현될 데이터로 가공한다.
		/// </summary>
		/// <param name="dt_GridSource">가공될 DataTable</param>
		/// <returns>가공 완료된 DataTable</returns>
		private DataTable ProcessGridSource( DataTable dt_GridSource )
		{
			for ( int i = 0 ; i < dt_GridSource.Rows.Count ; i = i + 3 )
			{
				...
				{
					decimal S_Cost = Convert.ToDecimal(dt_GridSource.Rows[i+1][j]);
					decimal P_Cost = Convert.ToDecimal(dt_GridSource.Rows[i+2][j]);
							
					try	
					{
						// 손실율 구하기 : 부적합금액 / 총금액 * 100
						dt_GridSource.Rows[i][j] = (decimal)((S_Cost / P_Cost) * 100.00m);
					}
					catch		// 0 으로 나누려 했을 시 걸림
					{
		...
		/// <summary>
		/// Grid에 바인딩 된 데이터를 챠트에 표현될 데이터로 가공한다.
		/// </summary>
		/// <param name="dt_ChartSource">가공될 DataTable(Grid Source)</param>
		/// <returns>가공 완료된 DataTable</returns>
