...
namespace KIT_ERP.SystemInfoManagement
{
	...
	public class WebForm2 : System.Web.UI.Page
	{
		...
		private void pageLoad()
		{
			...
			string str = "SELECT Title, Competence00,Competence01,Competence02,Competence03,Competence04,Competence05 From CT_T";
			...
			{
				if(Convert.ToBoolean(ds.Tables[0].Rows[i]["Competence00"]))
					UltraWebGrid1.Rows[i].Cells.FromKey("00 권한").Value = true;
				if(Convert.ToBoolean(ds.Tables[0].Rows[i]["Competence01"]))
					UltraWebGrid1.Rows[i].Cells.FromKey("Competence01").Value = true;
				if(Convert.ToBoolean(ds.Tables[0].Rows[i]["Competence02"]))
					UltraWebGrid1.Rows[i].Cells.FromKey("Competence02").Value = true;
				if(Convert.ToBoolean(ds.Tables[0].Rows[i]["Competence03"]))
					UltraWebGrid1.Rows[i].Cells.FromKey("Competence03").Value = true;
				if(Convert.ToBoolean(ds.Tables[0].Rows[i]["Competence04"]))
					UltraWebGrid1.Rows[i].Cells.FromKey("Competence04").Value = true;
				if(Convert.ToBoolean(ds.Tables[0].Rows[i]["Competence05"]))
					UltraWebGrid1.Rows[i].Cells.FromKey("Competence05").Value = true;
			}
		...
		private void Search()
		{
			...
			string str = @"Select [Index], Title as 페이지명, Competence00 as [00],
				...
				From CT_T Where [Group] Like @module";

			SqlConnection conn = new SqlConnection(ConfigurationSettings.AppSettings["Day"]);
			SqlCommand comm = new SqlCommand(str,conn);

			if(Module == "")
				comm.Parameters.Add("@module",SqlDbType.VarChar).Value = "%%";
			else
