...
{
	/// <summary>
	/// SubFreeBoard에 대한 요약 설명입니다.
	/// </summary>
	public class SubFreeBoard : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Response.Cookies["SFB_T"]["UpdateHits"] = "0";
			// SFB_T라는 쿠키값이 존재하면 pageNumber값을 iPage변수에 저장하고
			if(Request.Cookies["SFB_T"] != null)
			{
				string pageNumber = Request.Cookies["SFB_T"]["PageNumber"];				
				if(pageNumber != null)
			...
			if(!IsPostBack)
			{
				//Response.Write("<script language='javascript'>document.parentWindow.parent.ContentsTitle.location = '../ContentsTitle.aspx?title=∵ 협력사정보 > 자유게시판';</script>");
		...
		private void Listing()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT TOP 15 SubFreeBoardIndex, Title, Contents, RegistrationPerson, Convert(varchar, SFB_T.RegistrationDate, 111)RegistrationDate, Hits, Depth FROM SFB_T WHERE SubFreeBoardIndex NOT IN (SELECT TOP "+iPage*15+" SubFreeBoardIndex FROM SFB_T ORDER BY SubFreeBoardIndex desc) ORDER BY SubFreeBoardIndex desc";
		...
		// SFB_T 테이블의 총 레코드 수 구하기
		private int GetTotalRecordCount()
		{
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string strSql = "SELECT Count(*) FROM SFB_T";
		...
		private void DataGrid1_PageIndexChanged(object source, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			Response.Cookies["SFB_T"]["PageNumber"] = iPage.ToString();
			Listing();
		}
		...
		{
			Response.Redirect("SubFreeRegistration.aspx");
		}
