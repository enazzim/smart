...
{
	/// <summary>
	/// SubNotice에 대한 요약 설명입니다.
	/// </summary>
	public class SubNotice : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Response.Cookies["SN_T"]["UpdateHits"] = "0";
			
			if(Request.Cookies["SN_T"] != null)
			{
				string pageNumber = Request.Cookies["SN_T"]["PageNumber"];				
				if(pageNumber != null)
			...
			if(!IsPostBack)
			{
				//Response.Write("<script language='javascript'>document.parentWindow.parent.ContentsTitle.location = '../ContentsTitle.aspx?title=∵ 협력사정보 > 공지사항';</script>");
		...
		private void Listing()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT TOP 15 SubNoticeIndex, Title, Contents, RegistrationPerson, Convert(varchar, SN_T.RegistrationDate, 111)RegistrationDate, Hits FROM SN_T WHERE SubNoticeIndex NOT IN (SELECT TOP "+iPage*15+" SubNoticeIndex FROM SN_T ORDER BY SubNoticeIndex DESC) ORDER BY SubNoticeIndex desc";
		...
		// SN_T 테이블의 총 레코드 수 구하기
		private int GetTotalRecordCount()
		{
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string strSql = "SELECT Count(*) FROM SN_T";
		...
		private void DataGrid1_PageIndexChanged(object source, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			Response.Cookies["SN_T"]["PageNumber"] = iPage.ToString();
			Listing();
		}
		...
		{
			Response.Redirect("SubRegistration.aspx");
		}
