...
{
	/// <summary>
	/// SubAdjustment에 대한 요약 설명입니다.
	/// </summary>
	public class SubAdjustment : System.Web.UI.Page
	{
		...
		private string SubNoticeIndex = "";    // 현재 선택된 글의 번호
		private string updateHits = "0";
		protected System.Web.UI.WebControls.ValidationSummary Validationsummary2;     // 현재글의 조회수
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			SubNoticeIndex = Request.QueryString["SubNoticeIndex"];
			
			if(SubNoticeIndex == null)
				Response.Redirect("SubNotice.aspx");

			if(Request.Cookies["SN_T"] != null)
				updateHits = Request.Cookies["SN_T"]["UpdateHits"];
		...
		private void LoadData()
		{
			...
			Con.Open();
			Cmd.Parameters.Add("@SubNoticeIndex", SqlDbType.Int).Value = SubNoticeIndex;			

			// 조회수 증가
			if(updateHits == "0")
			{
				Cmd.CommandText = "UPDATE SN_T SET Hits = Hits + 1 WHERE SubNoticeIndex = @SubNoticeIndex";				
				Cmd.ExecuteNonQuery();
				Response.Cookies["SN_T"]["UpdateHits"] = "1";
			}

			Cmd.CommandText = "SELECT * FROM SN_T WHERE SubNoticeIndex = @SubNoticeIndex";
			...
			int Pre = 0, Next = 0;
			SqlCommand comm = new SqlCommand("SELECT TOP 1 SubNoticeIndex FROM SN_T WHERE SubNoticeIndex < @SubNoticeIndex ORDER BY SubNoticeIndex DESC", conn);
			comm.Parameters.Add("@SubNoticeIndex", SqlDbType.Int).Value = Request.QueryString["SubNoticeIndex"];
			...
			comm = new SqlCommand("SELECT TOP 1 SubNoticeIndex FROM SN_T WHERE SubNoticeIndex > @SubNoticeIndex ORDER BY SubNoticeIndex ASC", conn);
			comm.Parameters.Add("@SubNoticeIndex", SqlDbType.Int).Value = Request.QueryString["SubNoticeIndex"];
			if(comm.ExecuteScalar() == null)
			{
			...
			hlnkPreContent.NavigateUrl = "SubAdjustment.aspx?SubNoticeIndex=" + Pre.ToString();
			hlnkNextContent.NavigateUrl = "SubAdjustment.aspx?SubNoticeIndex=" + Next.ToString();
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			...
			Cmd.CommandText = "DELETE FROM SN_T WHERE SubNoticeIndex = @SubNoticeIndex";
			Cmd.Parameters.Add("@SubNoticeIndex", SqlDbType.Int);
			Cmd.Parameters["@SubNoticeIndex"].Value = SubNoticeIndex;
		...
		private void btnEdit_Click(object sender, System.EventArgs e)
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = "UPDATE SN_T SET Title=@Title, Contents=@Contents, RegistrationDate=@RegistrationDate WHERE SubNoticeIndex=@SubNoticeIndex";
