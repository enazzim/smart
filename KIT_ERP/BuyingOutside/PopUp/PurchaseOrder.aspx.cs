...
{
	/// <summary>
	/// PurchaseOrder에 대한 요약 설명입니다.
	/// </summary>
	public class PurchaseOrder : System.Web.UI.Page
	{
...
*/
		...
		private decimal Cost(string ItemNum, string Begin, string End)
		{
			decimal UnitCost = 0 ;
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			...
			Cmd.Connection = Con;
			Con.Open();
			Cmd.CommandText = "SELECT StandardUnitCost FROM UCI_MT Where BeginProcessCode = @BeginProcess and EndProcessCode = @EndProcess and ITemNum = @ItemNum and RecodingState = 1 and UnitCostDistinction = '외주단가'";				
			Cmd.Parameters.Add("@ItemNum",ItemNum);
			Cmd.Parameters.Add("@BeginProcess",Begin);
			Cmd.Parameters.Add("@EndProcess",End);
			SqlDataReader reader = Cmd.ExecuteReader(CommandBehavior.CloseConnection);
			
			if(reader.Read())
			{
				UnitCost = decimal.Parse(reader["StandardUnitCost"].ToString());
			}
		...
		private void First()
		{
			...
			Label7.Text = String.Format("{0:#,###}",uwg.Rows[0].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			UnitCost = Cost(uwg.Rows[0].Cells.FromKey("ItemNum").ToString(),uwg.Rows[0].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[0].Cells.FromKey("EndProcessCode").ToString());
...
*/
			...
			//Label89.Text = String.Format("{0:#,###}",UnitCost);
			//Label90.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[0].Cells.FromKey("OrderQuantity").Text));
			
			Label123.Text = uwg.Rows[0].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label112.Text = uwg.Rows[0].Cells.FromKey("EndProcess").ToString();			// 종료공정
		}
		...
		private void Second()
		{
			...
			Label15.Text = String.Format("{0:#,###}",uwg.Rows[1].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label124.Text = uwg.Rows[1].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label113.Text = uwg.Rows[1].Cells.FromKey("EndProcess").ToString();			// 종료공정
			UnitCost = Cost(uwg.Rows[1].Cells.FromKey("ItemNum").ToString(),uwg.Rows[1].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[1].Cells.FromKey("EndProcessCode").ToString());
			//Label92.Text = String.Format("{0:#,###}",UnitCost);
			//Label93.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[1].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Third()
		{
			...
			Label23.Text = String.Format("{0:#,###}",uwg.Rows[2].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label125.Text = uwg.Rows[2].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label114.Text = uwg.Rows[2].Cells.FromKey("EndProcess").ToString();			// 종료공정
			UnitCost = Cost(uwg.Rows[2].Cells.FromKey("ItemNum").ToString(),uwg.Rows[2].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[2].Cells.FromKey("EndProcessCode").ToString());
			//Label94.Text = String.Format("{0:#,###}",UnitCost);
			//Label95.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[2].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Fourth()
		{
			...
			Label31.Text = String.Format("{0:#,###}",uwg.Rows[3].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label126.Text = uwg.Rows[3].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label115.Text = uwg.Rows[3].Cells.FromKey("EndProcess").ToString();			// 종료공정
			UnitCost = Cost(uwg.Rows[3].Cells.FromKey("ItemNum").ToString(),uwg.Rows[3].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[3].Cells.FromKey("EndProcessCode").ToString());
			//Label96.Text = String.Format("{0:#,###}",UnitCost);
			//Label97.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[3].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Fifth()
		{
			...
			Label39.Text = String.Format("{0:#,###}",uwg.Rows[4].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label127.Text = uwg.Rows[4].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label116.Text = uwg.Rows[4].Cells.FromKey("EndProcess").ToString();			// 종료공정
			UnitCost = Cost(uwg.Rows[4].Cells.FromKey("ItemNum").ToString(),uwg.Rows[4].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[4].Cells.FromKey("EndProcessCode").ToString());
			//Label98.Text = String.Format("{0:#,###}",UnitCost);
			//Label99.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[4].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Sixth()
		{
			...
			Label47.Text = String.Format("{0:#,###}",uwg.Rows[5].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label128.Text = uwg.Rows[5].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label117.Text = uwg.Rows[5].Cells.FromKey("EndProcess").ToString();			// 종료공정

			UnitCost = Cost(uwg.Rows[5].Cells.FromKey("ItemNum").ToString(),uwg.Rows[5].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[5].Cells.FromKey("EndProcessCode").ToString());
			//Label100.Text = String.Format("{0:#,###}",UnitCost);
			//Label101.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[5].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Seventh()
		{
			...
			Label55.Text = String.Format("{0:#,###}",uwg.Rows[6].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label129.Text = uwg.Rows[6].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label118.Text = uwg.Rows[6].Cells.FromKey("EndProcess").ToString();			// 종료공정

			UnitCost = Cost(uwg.Rows[6].Cells.FromKey("ItemNum").ToString(),uwg.Rows[6].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[6].Cells.FromKey("EndProcessCode").ToString());
			//Label102.Text = String.Format("{0:#,###}",UnitCost);
			//Label103.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[6].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Eighth()
		{
			...
			Label63.Text = String.Format("{0:#,###}",uwg.Rows[7].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label130.Text = uwg.Rows[7].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label119.Text = uwg.Rows[7].Cells.FromKey("EndProcess").ToString();			// 종료공정

			UnitCost = Cost(uwg.Rows[7].Cells.FromKey("ItemNum").ToString(),uwg.Rows[7].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[7].Cells.FromKey("EndProcessCode").ToString());
			//Label104.Text = String.Format("{0:#,###}",UnitCost);
			//Label105.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[7].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Ninth()
		{
			...
			Label71.Text = String.Format("{0:#,###}",uwg.Rows[8].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label131.Text = uwg.Rows[8].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label120.Text = uwg.Rows[8].Cells.FromKey("EndProcess").ToString();			// 종료공정

			UnitCost = Cost(uwg.Rows[8].Cells.FromKey("ItemNum").ToString(),uwg.Rows[8].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[8].Cells.FromKey("EndProcessCode").ToString());
			//Label106.Text = String.Format("{0:#,###}",UnitCost);
			//Label107.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[8].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Tenth()
		{
			...
			Label79.Text = String.Format("{0:#,###}",uwg.Rows[9].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label132.Text = uwg.Rows[9].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label121.Text = uwg.Rows[9].Cells.FromKey("EndProcess").ToString();			// 종료공정

			UnitCost = Cost(uwg.Rows[9].Cells.FromKey("ItemNum").ToString(),uwg.Rows[9].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[9].Cells.FromKey("EndProcessCode").ToString());
			//Label108.Text = String.Format("{0:#,###}",UnitCost);
			//Label109.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[9].Cells.FromKey("OrderQuantity").Text));
		}
		...
		private void Eleventh()
		{
			...
			Label87.Text = String.Format("{0:#,###}",uwg.Rows[10].Cells.FromKey("OrderQuantity").Value);	// 총금액
			...
			Label133.Text = uwg.Rows[10].Cells.FromKey("BeginProcess").ToString();			// 시작공정
			Label122.Text = uwg.Rows[10].Cells.FromKey("EndProcess").ToString();			// 종료공정

			UnitCost = Cost(uwg.Rows[10].Cells.FromKey("ItemNum").ToString(),uwg.Rows[10].Cells.FromKey("BeginProcessCode").ToString(),uwg.Rows[10].Cells.FromKey("EndProcessCode").ToString());
			//Label110.Text = String.Format("{0:#,###}",UnitCost);
			//Label111.Text = String.Format("{0:#,###}",UnitCost*decimal.Parse(uwg.Rows[10].Cells.FromKey("OrderQuantity").Text));
		}
