...
namespace KIT_ERP.BuyingOutside
{
	...
	public class PaymentPlanResultRegistraton : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.TextBox BillMoney;
		protected System.Web.UI.WebControls.TextBox PaymentMoney;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				...
				BillMoney.Attributes.Add("OnKeyDown", "OnKeyDown_Float(this);");
				// onkeyup 이벤트가 발생하면 콤마를 찍고
				BillMoney.Attributes.Add("OnKeyUp", "Process(); OnKeyUp_Currency(this);");

				// focus를 얻으면 자동으로 select()
				BillMoney.Attributes.Add("OnFocus", "OnFocus_Obj(this);");

				// focus를 잃으면 다시 입력란을 체크
				BillMoney.Attributes.Add("OnBlur", "OnBlur_Cur(this);");

				PaymentMoney.Attributes.Add("OnKeyDown","OnKeyDown_Float(this);");
				PaymentMoney.Attributes.Add("OnKeyUp", "OnKeyUp_Currency(this)");
				PaymentMoney.Attributes.Add("onblur","OnBlur_Cur(this)");
				PaymentMoney.Attributes.Add("onfocus","OnFocus_Obj(this)");
		...
		private void Clear()
		{
			...
			this.BillMoney.Text = Convert.ToString(0); //수금액초기화
			this.PaymentMoney.Text = Convert.ToString(0);//지급액 초기화
			this.txtEvidence.Text = Convert.ToString(0);//증거자료 초기화
		...
		{			
			...
			ArrayPayment.Add(this.BillMoney.Text);//수금액(물품대)
			ArrayPayment.Add(this.PaymentMoney.Text);//지금액(부가세)
			if(SettleDropDown.SelectedItem.Value.Trim() == "")
			{
				ArrayPayment.Add(this.SettleDropDown.SelectedItem.Value);//결제방법코드
				ArrayPayment.Add("");//결제방법	
			}
			else
			{
				ArrayPayment.Add(this.SettleDropDown.SelectedItem.Value);//결제방법코드
		...
		private void btnEdit_Click(object sender, System.EventArgs e)
		{
			...
			EditArray.Add(this.BillMoney.Text);//수금액
			EditArray.Add(this.PaymentMoney.Text);//지급액
			if(SettleDropDown.SelectedItem.Value.Trim() == "")
			{
		...
		private void LinkButton1_Click(object sender, System.EventArgs e)
		{
			...
			while(dr.Read())
			{
				...
				WebNumericEdit1.Text = dr["UnPaymentMoney12"].ToString();
