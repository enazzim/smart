	...
	public class OutSideOrderPC : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				...
				btnDelete.Attributes.Add("onClick","return Confirm('선택한 품목들의 대하여 발주를 삭제 하시겠습니까?');");
				// 취소버튼에 대하여 onclick속성을 추가 시킴
				btnCancel.Attributes.Add("onClick","return Confirm('발주를 취소 하시겠습니까?');");
		...
		//Row Template 에서 수정을 하고 수정버튼을 누르면 LinkButton이 눌려진것처럼.....동작한다.
		private void linkUpdate_Click(object sender, System.EventArgs e)
		{
			...
			try
			{			
				OO_HT_Update(conn,tr); //발주원장 수정하는 함수
		...
		private void btEnd_Click(object sender, System.EventArgs e)
		{
			if ( this.SelectedRow_IsEnd() )
			{
				...
				for( int i = 0 ; i < uwgOO_HT.Rows.Count ; i ++)
				{
					...
					{
						int index = int.Parse(uwgOO_HT.Rows[i].Cells.FromKey("OutSideOrderHistoryIndex").Value.ToString());		// 원장인덱스
		...
		private bool SelectedRow_IsEnd()
		{
			...
			{
				if (Convert.ToBoolean( uwgOO_HT.Rows[i].Cells.FromKey("chk").Value ) 
					&& (uwgOO_HT.Rows[i].Cells.FromKey("ProgressCondition").Value.ToString() == "중단" || uwgOO_HT.Rows[i].Cells.FromKey("ProgressCondition").Value.ToString() == "완료"))
				{
					bReturnValue = false;
					break;
				}
		...
		private void DB_End(int idx)
		{
			...
			try
			{
				...
				string str = "Update OO_HT Set ProgressCondition = '완료', UpdatingPerson = @Person, UpdatingPersonID = @PersonID, UpdatingDate = @date  where OutSideOrderHistoryIndex = @OutSideOrderHistoryIndex";
		...
		private bool SelectedRow_IsEndCancle()
		{
			...
			for( int i = 0 ; i < uwgOO_HT.Rows.Count ; i ++)
			{
				if (Convert.ToBoolean( uwgOO_HT.Rows[i].Cells.FromKey("chk").Value) == true 
					&& uwgOO_HT.Rows[i].Cells.FromKey("ProgressCondition").Value.ToString() != "완료")
				{
					bReturnValue = false;
					break;
				}
		...
		private void DB_EndCancle(int idx)
		{
			...
			if(order - remain == 0 || order - (order - remain) < 0 )
				str = "Update OO_HT Set ProgressCondition = '대기',UpdatingPerson = @Person, UpdatingPersonID = @PersonID, UpdatingDate = @date  where OutSideOrderHistoryIndex = @OutSideOrderHistoryIndex ";
			else
				str = "Update OO_HT Set ProgressCondition = '진행',UpdatingPerson = @Person, UpdatingPersonID = @PersonID, UpdatingDate = @date  where OutSideOrderHistoryIndex = @OutSideOrderHistoryIndex";
