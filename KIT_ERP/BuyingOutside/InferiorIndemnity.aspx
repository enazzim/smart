...
<%@ Page language="c#" Codebehind="InferiorIndemnity.aspx.cs" AutoEventWireup="false" Inherits="KIT_ERP.BuyingOutside.InferiorIndemnity" %>
...
<HTML>
	<HEAD>
		...
		function ResettxtBox()
		{
			...
			document.InferiorIndemnity.ddlInferiorCause.options[0].selected=true;
			document.InferiorIndemnity.ddlInferiorStatus.options[0].selected=true;		
			document.InferiorIndemnity.lb_OldCost.value = 0;
			document.InferiorIndemnity.lb_Index.value = 0;
			document.InferiorIndemnity.tbQuantity.value = "0";
			document.InferiorIndemnity.tb_Cost.value = "0";
		}
		...
		function UltraWebGrid1_DblClickHandler(gridName, cellId){
			...
			document.InferiorIndemnity.ItemSearchControl1_txtItemNum.value = row.getCellFromKey("ItemNum").getValue();
			document.InferiorIndemnity.ItemSearchControl1_txtItemName.value = row.getCellFromKey("ItemName").getValue();			
			document.InferiorIndemnity.ItemSearchControl1_txtItemDrawNum.value = row.getCellFromKey("ItemDrawNum").getValue();
			document.InferiorIndemnity.CSC1_txtCompanyName.value = row.getCellFromKey("CompanyName").getValue();
			document.InferiorIndemnity.CSC1_txtBusinessRegistrationNum.value = row.getCellFromKey("BusinessRegistrationNum").getValue();

			combo2.setValue(row.getCellFromKey("ReceiptDate").getValue());//등록일
			
			document.InferiorIndemnity.lbItemNum.value = row.getCellFromKey("ItemDrawNum").getValue()	//품목번호
			document.InferiorIndemnity.lbItemNum.value = row.getCellFromKey("ItemNum").getValue();		//품목번호
			document.InferiorIndemnity.tb_Cost.value = row.getCellFromKey("ClaimCost").getValue();		//금액
			document.InferiorIndemnity.tbQuantity.value = row.getCellFromKey("ClaimQuantity").getValue();//수량
			document.InferiorIndemnity.lb_OldCost.value = row.getCellFromKey("ClaimCost").getValue();//기존금액
			
			
			//클레임현상
			for(var i=0;i<document.InferiorIndemnity.ddlClaimStatus.options.length;i++)
			{
				if(document.InferiorIndemnity.ddlClaimStatus.options[i].innerText == row.getCellFromKey("ClaimStatusMeaning").getValue())
				{
					document.InferiorIndemnity.ddlClaimStatus.options[i].selected=true;
					break;
				}
				else
					document.InferiorIndemnity.ddlClaimStatus.options[0].selected=true;
			}
			
			//클레임원인
			for(var i=0;i<document.InferiorIndemnity.ddlClaimCause.options.length;i++)
			{
				if(document.InferiorIndemnity.ddlClaimCause.options[i].innerText == row.getCellFromKey("ClaimCauseMeaning").getValue())
				{
					document.InferiorIndemnity.ddlClaimCause.options[i].selected=true;
					break;
				}
				else
					document.InferiorIndemnity.ddlClaimCause.options[0].selected=true;
			}
			
		    document.InferiorIndemnity.lb_Index.value = row.getCellFromKey("ClameHistoryIndex").getValue();//원장인덱스
		    
		    
		      
		    document.InferiorIndemnity.bt_Update.disabled = false;
		    document.InferiorIndemnity.bt_Delete.disabled = false;
		    document.InferiorIndemnity.bt_Register.disabled = true;
		}
...
--></SCRIPT>
	...
	<body bgColor="#f7f6f6" MS_POSITIONING="GridLayout" XMLNS:igtbl="http://schemas.infragistics.com/ASPNET/WebControls/UltraWebGrid">
		<form id="InferiorIndemnity" method="post" runat="server">
					...
					<TD style="PADDING-TOP: 10px" vAlign="top" align="center" width="800">
						<FIELDSET style="BORDER-RIGHT: #696969 2px solid; BORDER-TOP: #696969 2px solid; BORDER-LEFT: #696969 2px solid; WIDTH: 100%; BORDER-BOTTOM: #696969 2px solid">
							...
							<TABLE id="Table3" cellSpacing="0" cellPadding="0" width="800" border="0">
								<TR>
									<TD align="left" width="210" height="25" colSpan="2">
								...
								<TR>
									...
									<TD width="330" height="25">
										<asp:textbox id="tb_Cost" style="TEXT-ALIGN: right" runat="server" Height="20px" Width="100px"
											BackColor="#EEEEE9" BorderStyle="Inset">0</asp:textbox><INPUT id="lb_OldCost" style="WIDTH: 50px; HEIGHT: 20px" type="hidden" value="0" name="Hidden1"
											runat="server"></TD>
								...
								<TR>
									...
									<TD width="110" height="25">
										<asp:dropdownlist id="ddlInferiorStatus" runat="server" Font-Size="9pt" Height="20px" Width="100px"
											BackColor="#EEEEE9"></asp:dropdownlist></TD>
									<TD style="FONT-SIZE: 9pt" align="right" width="70" height="25"><FONT style="FONT-SIZE: 9pt" face="굴림">불량원인&nbsp;
										</FONT>
									</TD>
									<TD width="130" height="25">
										<asp:dropdownlist id="ddlInferiorCause" runat="server" Font-Size="9pt" Height="20px" Width="105px"
											BackColor="#EEEEE9"></asp:dropdownlist></TD>
								...
								<Bands>
									<igtbl:UltraGridBand>
										<Columns>
											...
											<igtbl:UltraGridColumn HeaderText="불량수량" Key="InferiorQuantity" Format="###,###,###.##" HeaderClickAction="SortMulti"
												BaseColumnName="InferiorQuantity">
											...
											<igtbl:UltraGridColumn HeaderText="불량 사유" Key="InferiorStatusMeaning" HeaderClickAction="SortMulti" BaseColumnName="InferiorStatusMeaning">
												<CellStyle VerticalAlign="Middle" HorizontalAlign="Center"></CellStyle>
												<HeaderStyle VerticalAlign="Middle" HorizontalAlign="Center"></HeaderStyle>
											</igtbl:UltraGridColumn>
											<igtbl:UltraGridColumn HeaderText="불량 사유코드" Key="InferiorStatusCode" Hidden="True" HeaderClickAction="SortMulti"
												BaseColumnName="InferiorStatusCode">
												<CellStyle VerticalAlign="Middle" HorizontalAlign="Center"></CellStyle>
												<HeaderStyle VerticalAlign="Middle" HorizontalAlign="Center"></HeaderStyle>
											</igtbl:UltraGridColumn>
											<igtbl:UltraGridColumn HeaderText="불량 원인" Key="InferiorCauseMeaning" HeaderClickAction="SortMulti" BaseColumnName="InferiorCauseMeaning">
												<CellStyle VerticalAlign="Middle" HorizontalAlign="Center"></CellStyle>
												<HeaderStyle VerticalAlign="Middle" HorizontalAlign="Center"></HeaderStyle>
											</igtbl:UltraGridColumn>
											<igtbl:UltraGridColumn HeaderText="불량 원인코드" Key="InferiorCauseCode" Hidden="True" HeaderClickAction="SortMulti"
												BaseColumnName="InferiorCauseCode">
												<CellStyle VerticalAlign="Middle" HorizontalAlign="Center"></CellStyle>
												<HeaderStyle VerticalAlign="Middle" HorizontalAlign="Center"></HeaderStyle>
											</igtbl:UltraGridColumn>
