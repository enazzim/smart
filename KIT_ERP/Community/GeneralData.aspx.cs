...
{
	/// <summary>
	/// GeneralData에 대한 요약 설명입니다.
	/// </summary>
	public class GeneralData : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Response.Cookies["CGD_T"]["UpdateHits"] = "0";
			// CGD_T라는 쿠키값이 존재하면 pageNumber값을 iPage변수에 저장하고
			if(Request.Cookies["CGD_T"] != null)
			{
				string pageNumber = Request.Cookies["CGD_T"]["PageNumber"];				
				if(pageNumber != null)
			...
			if(!IsPostBack)
			{
				//Response.Write("<script language='javascript'>document.parentWindow.parent.ContentsTitle.location = '../ContentsTitle.aspx?title=∵ 커뮤니티 > 일반자료실';</script>");
		...
		private void Listing()
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT TOP 15 CGeneralDataIndex, Title, Contents, FileName, RegistrationPerson, Convert(varchar, CGD_T.RegistrationDate, 111)RegistrationDate, Hits FROM CGD_T WHERE CGeneralDataIndex NOT IN (SELECT TOP "+iPage*15+" CGeneralDataIndex FROM CGD_T ORDER BY CGeneralDataIndex desc) ORDER BY CGeneralDataIndex desc";
		...
		// CGD_T 테이블의 총 레코드 수 구하기
		private int GetTotalRecordCount()
		{
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string strSql = "SELECT Count(*) FROM CGD_T";
		...
		private void DataGrid1_PageIndexChanged_1(object source, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			Response.Cookies["CGD_T"]["PageNumber"] = iPage.ToString();
			Listing();		
		}
		...
		{
			Response.Redirect("GnRegistration.aspx");
		}		
		...
		private void Button1_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				try
				{
					Cmd.CommandText = "DELETE FROM CCG_T WHERE Category = @Category AND Classify = 3";
					Cmd.Parameters.Add("@Category", SqlDbType.VarChar).Value = DropDownList1.SelectedItem.Text;
					Cmd.ExecuteNonQuery();
					
					Cmd.CommandText = "DELETE FROM CGD_T WHERE Category = @Category2";
					Cmd.Parameters.Add("@Category2", SqlDbType.VarChar).Value = DropDownList1.SelectedItem.Text;
					Cmd.ExecuteNonQuery();
					
					trans.Commit();
					Con.Close();	
					Response.Write("<script>alert('삭제되었습니다.');location.href='GeneralData.aspx'; </script>");
				}
				catch(Exception ex)
				{
		...
		private void DropDownList1_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			if(DropDownList1.SelectedIndex == 0)
			{
				...
				Cmd.CommandText = "SELECT TOP 10 CGeneralDataIndex, Title, Contents, RegistrationPerson, Convert(varchar, CGD_T.RegistrationDate, 111)RegistrationDate, Hits FROM CGD_T WHERE CGeneralDataIndex NOT IN(SELECT TOP "+iPage*10+" CGeneralDataIndex FROM CGD_T ORDER BY CGeneralDataIndex desc) ORDER BY CGeneralDataIndex desc";
			...
			else
			{
				...
				Cmd.CommandText = "SELECT TOP 10 CGeneralDataIndex, Title, Contents, RegistrationPerson, Convert(varchar, CGD_T.RegistrationDate, 111)RegistrationDate, Hits FROM CGD_T WHERE Category = @Category AND CGeneralDataIndex NOT IN(SELECT TOP "+iPage*10+" CGeneralDataIndex FROM CGD_T WHERE Category = @Category ORDER BY CGeneralDataIndex desc) ORDER BY CGeneralDataIndex desc";
			...
			DataGrid1.DataKeyField = "CGeneralDataIndex"; 
			DataGrid1.DataBind();
			lblPageInfo.Text = string.Format("( 현재 {0}페이지 / 전체 {1}페이지 )", iPage+1, DataGrid1.PageCount);
		}	
		...
		private int GetSearchRecordCount()
		{
			...
			Cmd.CommandText = "SELECT Count(*) FROM CGD_T WHERE Category = @Category";			
			Cmd.Parameters.Add("@Category", SqlDbType.VarChar).Value = DropDownList1.SelectedItem.Text;
