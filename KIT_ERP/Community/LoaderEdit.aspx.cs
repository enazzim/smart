...
{
	/// <summary>
	/// LoaderEdit에 대한 요약 설명입니다.
	/// </summary>
	public class LoaderEdit : System.Web.UI.Page
	{
		...
		private bool Exist(string item)
		{
			...
			SqlCommand comm = new SqlCommand();
			comm.Connection = conn;
			comm.CommandText = "Select count(ID) From Relation_T where [ID] = @id and No = @no and Division = '로더생산현황'";
		...
		private void btnEdit_Click(object sender, System.EventArgs e)
		{
			...
			{
				if(File1.Value.Trim() == "")
					Cmd.CommandText = "UPDATE T_Loader SET Title=@Title, content=@content, transdate=@transdate WHERE seq=@seq";
				else
				{
					if(FileSize == 0)
						throw new Exception("파일크기가 0보다 커야 합니다!");
					Cmd.CommandText = "UPDATE T_Loader SET Title=@Title, content=@content, AppendFile=@AppendFile, FileName=@FileName, FileSize=@FileSize, FileType=@FileType, transdate=@transdate WHERE seq=@seq";
				}
