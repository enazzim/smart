...
{
	/// <summary>
	/// MaterialsContent에 대한 요약 설명입니다.
	/// </summary>
	public class MaterialsContent : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			MaterialsIndex = Request.QueryString["seq"];
		...
		private void Listing()
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT CRelationShipIndex, StandardinfoTableID, SITIndex, case CommunityDataTableID when 'T_Materials' then '자재관리' end as CommunityDataTableID , CDTIndex FROM CR_T WHERE CommunityDataTableID='T_Materials' AND CDTIndex=@MaterialsIndex ORDER BY CRelationShipIndex DESC";
			Cmd.Parameters.Add("@MaterialsIndex", SqlDbType.Int);			
			Cmd.Parameters["@MaterialsIndex"].Value = MaterialsIndex;
		...
		private void LoadData()
		{
			...
			if(reader.Read())
			{
				...
				HyperLink1.NavigateUrl = "Down.aspx?seq=" + Request.QueryString["seq"] + "&Division=Materials&filename=AppendFile";
			...
			comm.CommandText = "SELECT TOP 1 seq FROM T_Materials WHERE seq < @seq ORDER BY seq DESC";
			...
			comm = new SqlCommand("SELECT TOP 1 seq FROM T_Materials WHERE seq > @seq ORDER BY seq ASC", conn);
			comm.Parameters.Add("@seq", SqlDbType.Int).Value = Request.QueryString["seq"];
			if(comm.ExecuteScalar() == null)
			{
			...
			hlnkPreContent.NavigateUrl = "MaterialsContent.aspx?seq=" + Pre.ToString();
			hlnkNextContent.NavigateUrl = "MaterialsContent.aspx?seq=" + Next.ToString();
			...
			//필독자 바인딩
			comm.CommandText = @"SELECT Name + '(' + R.ID + ')'  as Name ,R.ID FROM UI_MT inner join Relation_T as R on UI_MT.ID = R.ID WHERE (RecodingState = 1) and Division='자재관리' and NO = @No ORDER BY Name";
			...
			comm.CommandText = "Select count(ID) From Relation_T where Division='자재관리' and NO = @No and [ID] = @id and ReadDate is null";
			comm.Parameters.Add("@No",seq);
			...
			{
				//
				comm.CommandText = "Update Relation_T Set ReadDate = @date where Division='자재관리' and NO = @No and [ID] = @id";
				comm.Parameters.Add("@No",seq);
				comm.Parameters.Add("@id",Session["ID"].ToString());
			...
			comm.CommandText = "SELECT Name + '(' + R.ID + ')'  as Name, R.ReadDate FROM UI_MT inner join Relation_T as R on UI_MT.ID = R.ID WHERE (RecodingState = 1) and Division='자재관리' and NO = @No and ReadDate is not null ORDER BY ReadDate, Name";
			comm.Parameters.Add("@No",seq);
		...
		private void btnReply_Click(object sender, System.EventArgs e)
		{
			...
			Response.Redirect("MaterialsWrite.aspx?thread=" + thread + "&depth=" + depth + "&seq=" + seq);
		}

		private void btnEdit_Click(object sender, System.EventArgs e)
		{
			Response.Redirect("MaterialsEdit.aspx?seq=" + ViewState["seq"].ToString());   
		}
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				Cmd.CommandText = "DELETE FROM T_Materials WHERE seq = @seq";							
				Cmd.Parameters.Add("@seq", SqlDbType.Int);
				Cmd.Parameters["@seq"].Value = seq;
				Cmd.ExecuteNonQuery();

				Cmd.CommandText = "Delete From Relation_T Where [No] = @seq and Division = '자재관리'";
				Cmd.ExecuteNonQuery();
		...
		{
			Response.Write("<script>window.open('Popup/CompanyInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"','CompanyInfo', 'resizable=1, scrollbars=1, width=850, height=530');</script>");
		}

		// 품목정보 팝업창
		private void btnItemInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/ItemInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"','ItemInfo', 'resizable=1, scrollbars=1, width=800, height=530');</script>");		
		}

		// 품목구성정보 팝업창
		private void btnItemOrganizationInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/ItemOrganizationInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"','ItemOrganizationInfo', 'resizable=1, scrollbars=1, width=700, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/EquipmentInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"','EquipmentInfo', 'resizable=1, scrollbars=1, width=550, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/WCInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"','WCInfo', 'resizable=1, scrollbars=1, width=710, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/ProcessSequenceInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"','ProcessSequenceInfo', 'resizable=1, scrollbars=1, width=800, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/WorkStandardInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"','WorkStandardInfo', 'resizable=1, scrollbars=1, width=800, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/SaleUnitCostInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'SaleUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/OutSideOrderUnitCostInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'OutSideOrderUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/BuyUnitCostInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'BuyUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/UserInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'UserInfo', 'resizable=1, scrollbars=1, width=730, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/PublicUseCode.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'PublicUseCode', 'resizable=1, scrollbars=1, width=790, height=800');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/BusinessPlanInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'BusinessPlanInfo', 'resizable=1, scrollbars=1, width=940, height=530');</script>");		
		}
		...
		{
			Response.Write("<script>window.open('Popup/ExecutionPlanInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'ExecutionPlanInfo', 'resizable=1, scrollbars=1, width=940, height=530');</script>");		
		}

		//실공정정보 팝업
		private void btnRealProcessSequenceInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/RealProcessSequenceInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'RealProcessSequenceInfo', 'resizable=1, scrollbars=1, width=700, height=530');</script>");		
		}

		//실품목구성 팝업
		private void btnRealItemOrganizationInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/RealItemOrganizationInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'RealItemOrganizationInfo', 'resizable=1, scrollbars=1, width=700, height=530');</script>");		
		}
		//실 작업표준 팝업
		private void btnRealWorkStandardInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/RealWorkStandardInfo.aspx?MaterialsIndex="+MaterialsIndex.ToString()+"', 'RealWorkStandardInfo', 'resizable=1, scrollbars=1, width=700, height=530');</script>");		
		}
		...
		private void Datagrid2_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{				
				...
				Cmd.ExecuteNonQuery();
				trans.Commit();								
				Response.Write("<script>alert('삭제 되었습니다.'); location.href='MaterialsContent.aspx?seq=" + MaterialsIndex + "';; </script>");
			}
			catch(Exception ex)
			{
				Response.Write(ex.Message);
