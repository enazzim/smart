	...
	public class PublicUseCode : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From PUC_MT where RecodingState = 1 ",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string str = @"select Top "+TopCount.ToString()+ @" PublicUseCodeIndex, LargeClassificationCode, LargeClassificationName, SmallClassificationCode, SmallClassificationName FROM PUC_MT Where RecodingState = 1 ORDER BY LargeClassificationCode ASC";

			SqlDataAdapter adp = new SqlDataAdapter(str,Con);
			DataSet ds = new DataSet();

			int StartRecord = DataGrid1.CurrentPageIndex * DataGrid1.PageSize;
			adp.Fill(ds,StartRecord, DataGrid1.PageSize, "PublicUseCode");

			adp.Fill(ds);
			DataGrid1.DataSource = ds;
			DataGrid1.DataMember = "PublicUseCode";
			DataGrid1.DataBind();
		}
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('공용코드정보', "+ item.Cells[1].Text +", 'CGD_T', "+ CGeneralDataIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('공용코드정보', "+ item.Cells[1].Text +", 'CTD_T', "+ CTechnologyDataIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('공용코드정보', "+ item.Cells[1].Text +", 'CCS_T', "+ CCompanyStandardIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
