...
{
	/// <summary>
	/// BuyUnitCostInfo에 대한 요약 설명입니다.
	/// </summary>
	public class BuyUnitCostInfo : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			ISC1.ChildICommodity  = true;
			ISC1.ChildRawMaterials = true;
			...
			if(!IsPostBack)
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From UCI_MT where RecodingState = 1 and (UCI_MT.UnitCostDistinction = '구매단가') ",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			{
				Cmd.CommandText = @"SELECT  Top "+TopCount.ToString()+ @"UCI_MT.UnitCostInfoIndex , UCI_MT.ItemNum, II_MT.ItemName, UCI_MT.UnitCostDistinction, CI_MT.CompanyName, PropertyClassification, Standard
						FROM UCI_MT
						inner join ii_mt 
						on ii_mt.ItemNum = uci_mt.itemnum 
						inner join ci_mt 
						on ci_mt.BusinessRegistrationNum = uci_mt.BusinessRegistrationNum 
						Where CI_MT.RecodingState = 1 and  UCI_MT.RecodingState = 1 and UCI_MT.UnitCostDistinction = '구매단가' and II_MT.RecodingState = 1 
			...
			adp.Fill(ds,StartRecord, DataGrid1.PageSize, "BuyUnitCost");

			adp.Fill(ds);
			DataGrid1.DataSource = ds;
			DataGrid1.DataMember = "BuyUnitCost";
			DataGrid1.DataBind();
		}
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('구매단가정보', "+ item.Cells[1].Text +", 'T_Materials', "+ MaterialsIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('구매단가정보', "+ item.Cells[1].Text +", 'T_Product', "+ ProductIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('구매단가정보', "+ item.Cells[1].Text +", 'T_Development', "+ DevelopmentIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
