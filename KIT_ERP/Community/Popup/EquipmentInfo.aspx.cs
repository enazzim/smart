...
//			CCompanyStandardIndex = Request.QueryString["CCompanyStandardIndex"];
			...
			if ( !IsPostBack )
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From EI_MT where RecodingState = 1 ",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string str = @"select Top "+TopCount.ToString()+ @" EquipmentInfoIndex, EquipmentNum, EquipmentName, EquipmentClassification, InstrumentNum FROM EI_MT Where RecodingState = 1 ORDER BY EquipmentName ASC";
			...
			adp.Fill(ds,StartRecord, DataGrid1.PageSize, "EquipmentInfo");

			adp.Fill(ds);
			DataGrid1.DataSource = ds;
			DataGrid1.DataMember = "EquipmentInfo";
			DataGrid1.DataBind();
		}
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('설비정보', "+ item.Cells[1].Text +", 'T_Materials', "+ MaterialsIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('설비정보', "+ item.Cells[1].Text +", 'T_Product', "+ ProductIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('설비정보', "+ item.Cells[1].Text +", 'T_Development', "+ DevelopmentIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
...
//						{
//							Cmd.CommandText = "INSERT CR_T VALUES('설비정보', "+ item.Cells[1].Text +", 'CGD_T', "+ CGeneralDataIndex +")";
//							Cmd.ExecuteNonQuery();
//							trans.Commit();
...
//						{
//							Cmd.CommandText = "INSERT CR_T VALUES('설비정보', "+ item.Cells[1].Text +", 'CTD_T', "+ CTechnologyDataIndex +")";
//							Cmd.ExecuteNonQuery();
//							trans.Commit();
...
//						else
//						{
//							Cmd.CommandText = "INSERT CR_T VALUES('설비정보', "+ item.Cells[1].Text +", 'CCS_T', "+ CCompanyStandardIndex +")";
//							Cmd.ExecuteNonQuery();
//							trans.Commit();
