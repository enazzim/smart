	...
	public class ExecutionPlanInfo : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From EPI_MT where RecodingState = 1",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string str = @"select Top "+TopCount.ToString()+ @" EPI_MT.ExecutionPlanInfoIndex AS ExecutionPlanInfoIndex, 
											II_MT.ItemName AS ItemName, EPI_MT.ItemNum AS ItemNum, 
											II_MT.ItemDrawNum AS ItemDrawNum, 
											II_MT.PropertyClassification AS PropertyClassification, II_MT.Unit AS Unit, 
											II_MT.Standard AS Standard, Convert(varchar, EPI_MT.PlanDate, 111) AS PlanDate, 
											EPI_MT.PlanQuantity AS PlanQuantity, EPI_MT.SaleUnitCost AS SaleUnitCost, 
											EPI_MT.PlanTotalCost AS PlanTotalCost
									FROM    EPI_MT INNER JOIN
											II_MT ON EPI_MT.ItemNum = II_MT.ItemNum Where EPI_MT.RecodingState = 1
											ORDER BY CONVERT(varchar, EPI_MT.PlanDate, 111) DESC";
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('실행계획정보', "+ item.Cells[1].Text +", 'CGD_T', "+ CGeneralDataIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('실행계획정보', "+ item.Cells[1].Text +", 'CTD_T', "+ CTechnologyDataIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('실행계획정보', "+ item.Cells[1].Text +", 'CCS_T', "+ CCompanyStandardIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
