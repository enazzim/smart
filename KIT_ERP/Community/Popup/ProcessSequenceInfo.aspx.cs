...
{
	/// <summary>
	/// ProcessSequenceInfo에 대한 요약 설명입니다.
	/// </summary>
	public class ProcessSequenceInfo : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From PSI_MT where RecodingState = 1",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText= @"select Top "+TopCount.ToString()+ @" PSI_MT.ProcessSequenceInfoIndex AS ProcessSequenceInfoIndex,
									PSI_MT.ItemNum AS ItemNum, 
									II_MT.ItemName AS ItemName,
									PSI_MT.ProcessSequenceNum AS ProcessSequenceNum, 
									PUC_MT.SmallClassificationName AS SmallClassificationName, 
									PSI_MT.WorkDistinction AS WorkDistinction
								...
								WHERE   (PSI_MT.RecodingState = 1) and PUC_MT.RecodingState = 1 and II_MT.RecodingState = 1
			...
			adp.Fill(ds,StartRecord, DataGrid1.PageSize, "ProcessSequenceInfo");

			adp.Fill(ds);
			DataGrid1.DataSource = ds;
			DataGrid1.DataMember = "ProcessSequenceInfo";
			DataGrid1.DataBind();
		}
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('공정순서정보', "+ item.Cells[1].Text +", 'T_Materials', "+ MaterialsIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('공정순서정보', "+ item.Cells[1].Text +", 'T_Product', "+ ProductIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('공정순서정보', "+ item.Cells[1].Text +", 'T_Development', "+ DevelopmentIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
		...
		private void btSearch_Click(object sender, System.EventArgs e)
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = @"select Count(ProcessSequenceInfoIndex) FROM PSI_MT inner join II_MT on PSI_MT.ItemNum = II_MT.ItemNum
				WHERE PSI_MT.RecodingState = 1 and II_MT.RecodingState = 1
				and II_MT.ItemNum Like @ItemNum and II_MT.ItemDrawNum like @ItemDrawNum and II_MT.ItemName Like @ItemName";
			Cmd.Parameters.Add("@ItemNum","%"+ISC1.ChildItemNum+"%");
			Cmd.Parameters.Add("@ItemDrawNum","%"+ISC1.ChildItemDrawNum+"%");
			Cmd.Parameters.Add("@ItemName","%"+ISC1.ChildItemName+"%");
