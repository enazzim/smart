	...
	public class ItemOrganizationInfo : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From IOI_MT where RecodingState = 1",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText= @"select Top "+TopCount.ToString()+ @" ItemOrganizationInfoIndex, 
				ParentItemNum, ChildItemNum, NeedQuantityNumerator, NeedQuantityDenominator 
				FROM IOI_MT inner join II_MT II_MT1 on IOI_MT.ParentItemNum = II_MT1.ItemNum 
				inner join II_MT II_MT2 on IOI_MT.ChildItemNum = II_MT2.ItemNum
				WHERE IOI_MT.RecodingState = 1 and II_MT1.RecodingState = 1 and II_MT2.RecodingState = 1
			...
			adp.Fill(ds,StartRecord, DataGrid1.PageSize, "ItemOrganizationInfo");

			adp.Fill(ds);
			DataGrid1.DataSource = ds;
			DataGrid1.DataMember = "ItemOrganizationInfo";
			DataGrid1.DataBind();
		}
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('품목구성정보', "+ item.Cells[1].Text +", 'T_Materials', "+ MaterialsIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('품목구성정보', "+ item.Cells[1].Text +", 'T_Product', "+ ProductIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('품목구성정보', "+ item.Cells[1].Text +", 'T_Development', "+ DevelopmentIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
		...
		private void btSearch_Click(object sender, System.EventArgs e)
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = @"select Count(ItemOrganizationInfoIndex) FROM IOI_MT inner join II_MT II_MT1 on IOI_MT.ParentItemNum = II_MT1.ItemNum 
				inner join II_MT II_MT2 on IOI_MT.ChildItemNum = II_MT2.ItemNum
				WHERE IOI_MT.RecodingState = 1 and II_MT1.RecodingState = 1 and II_MT2.RecodingState = 1
				and II_MT1.ItemNum Like @ParentItemNum and II_MT1.ItemDrawNum like @ParentItemDrawNum and II_MT1.ItemName Like @ParentItemName
				and II_MT2.ItemNum Like @ChildItemNum and II_MT2.ItemDrawNum like @ChildItemDrawNum and II_MT2.ItemName Like @ChildItemName";
