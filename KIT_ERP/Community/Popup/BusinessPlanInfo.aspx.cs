	...
	public class BusinessPlanInfo : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From BPI_MT where RecodingState = 1",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string str = @"SELECT Top "+TopCount + @" BPI_MT.BusinessPlanInfoIndex AS BusinessPlanInfoIndex, 
											II_MT.ItemName AS ItemName, BPI_MT.ItemNum AS ItemNum, 
											II_MT.ItemDrawNum AS ItemDrawNum, 
											II_MT.PropertyClassification AS PropertyClassification, II_MT.Unit AS Unit, 
											II_MT.Standard AS Standard, BPI_MT.PlanQuantity AS PlanQuantity, 
											BPI_MT.PlanYear AS PlanYear, BPI_MT.PlanMonth AS PlanMonth, 
											BPI_MT.PlanDay AS PlanDay, CI_MT.CompanyName AS CompanyName, 
											BPI_MT.BusinessRegistrationNum AS BusinessRegistrationNum, 
											BPI_MT.SaleUnitCost AS SaleUnitCost, 
											BPI_MT.PlanTotalCost AS PlanTotalCost
									FROM    BPI_MT INNER JOIN
											CI_MT ON 
											BPI_MT.BusinessRegistrationNum = CI_MT.BusinessRegistrationNum INNER JOIN
											II_MT ON BPI_MT.ItemNum = II_MT.ItemNum Where BPI_MT.RecodingState = 1";

			SqlDataAdapter adp = new SqlDataAdapter(str,Con);
			DataSet ds = new DataSet();

			int StartRecord = DataGrid1.CurrentPageIndex * DataGrid1.PageSize;
			adp.Fill(ds,StartRecord, DataGrid1.PageSize, "BPI_MT");

			adp.Fill(ds);
			DataGrid1.DataSource = ds;
			DataGrid1.DataMember = "BPI_MT";
			DataGrid1.DataBind();
		}
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('사업계획정보', "+ item.Cells[1].Text +", 'CGD_T', "+ CGeneralDataIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('사업계획정보', "+ item.Cells[1].Text +", 'CTD_T', "+ CTechnologyDataIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('사업계획정보', "+ item.Cells[1].Text +", 'CCS_T', "+ CCompanyStandardIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
