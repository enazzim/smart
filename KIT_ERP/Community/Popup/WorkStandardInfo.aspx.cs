	...
	public class WorkStandardInfo : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From WSI_MT where RecodingState = 1 ",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = @"select Top "+TopCount.ToString()+ @" WSI_MT.WorkStandardInfoIndex AS WorkStandardInfoIndex, 
									 		 WSI_MT.ItemNum AS ItemNum, II_MT.ItemName AS ItemName,
									 		 WSI_MT.ProcessSequenceNum AS ProcessSequenceNum, 
											 PUC_MT.SmallClassificationName AS SmallClassificationName, 
											 WSI_MT.PriorityOrder AS PriorityOrder
									FROM     WSI_MT INNER JOIN
											 PUC_MT ON WSI_MT.ProcessCode = PUC_MT.SmallClassificationCode
											INNER JOIN II_MT
											ON WSI_MT.ItemNum = II_MT.ItemNum
									WHERE    (WSI_MT.RecodingState = 1) and PUC_MT.RecodingState = 1  and II_MT.RecodingState = 1
											and WSI_MT.ItemNum Like @ItemNum and II_MT.ItemDrawNum Like @ItemDrawNum and II_MT.ItemName Like @ItemName
									order by ItemNum, ProcessSequenceNum, PriorityOrder ";
			...
			adp.Fill(ds,StartRecord, DataGrid1.PageSize, "WorkStandardInfo");

			adp.Fill(ds);
			DataGrid1.DataSource = ds;
			DataGrid1.DataMember = "WorkStandardInfo";
			DataGrid1.DataBind();
		}
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('작업표준정보', "+ item.Cells[1].Text +", 'T_Materials', "+ MaterialsIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('작업표준정보', "+ item.Cells[1].Text +", 'T_Product', "+ ProductIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('작업표준정보', "+ item.Cells[1].Text +", 'T_Development', "+ DevelopmentIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
		...
		private void btSearch_Click(object sender, System.EventArgs e)
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = @"select Count(WorkStandardInfoIndex) FROM WSI_MT inner join II_MT on WSI_MT.ItemNum = II_Mt.ItemNum
							 Where WSI_MT.RecodingState = 1 and II_MT.ItemNum Like @ItemNum and II_MT.ItemDrawNum like @ItemDrawNum and II_MT.ItemName like @ItemName";
			Cmd.Parameters.Add("@ItemNum", "%"+ISC1.ChildItemNum+"%");
			Cmd.Parameters.Add("@ItemDrawNum", "%"+ISC1.ChildItemDrawNum+"%");
			Cmd.Parameters.Add("@ItemName", "%"+ISC1.ChildItemName+"%");
