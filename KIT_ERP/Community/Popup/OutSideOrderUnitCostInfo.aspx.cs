...
{
	/// <summary>
	/// OutSideOrderUnitCostInfo에 대한 요약 설명입니다.
	/// </summary>
	public class OutSideOrderUnitCostInfo : System.Web.UI.Page
	{
		...
		protected KIT_ERP.Community.CompanyControl.CompanyControl CSC1;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				SqlCommand Cmd = new SqlCommand("select count(*) From UCI_MT where RecodingState = 1 and (UCI_MT.UnitCostDistinction = '외주단가') ",Con);
				Con.Open();
		...
		private void DataGridBind()
		{
			...
			{
				Cmd.CommandText = @"SELECT  Top "+TopCount.ToString()+ @"UCI_MT.UnitCostInfoIndex , UCI_MT.ItemNum, II_MT.ItemName, UCI_MT.UnitCostDistinction, CI_MT.CompanyName,
						PUC_MT_1.SmallClassificationName as BeginProcess, PUC_MT_2.SmallClassificationName as EndProcess 
						...
						Where CI_MT.RecodingState = 1 and  UCI_MT.RecodingState = 1 and UCI_MT.UnitCostDistinction = '외주단가' and II_MT.RecodingState = 1 
						and II_MT.ItemNum Like @ItemNum and II_MT.ItemDrawNum Like @ItemDrawNum and II_MT.ItemName Like @ItemName and CI_MT.CompanyName Like @CompanyName
						order by II_MT.ItemNum ";
				Cmd.Parameters.Add("@CompanyName","%"+CSC1.Company+"%");
			}
			else
			{
				Cmd.CommandText = @"SELECT  Top "+TopCount.ToString()+ @"UCI_MT.UnitCostInfoIndex , UCI_MT.ItemNum, II_MT.ItemName, UCI_MT.UnitCostDistinction, CI_MT.CompanyName,
						...
						on ci_mt.BusinessRegistrationNum = uci_mt.BusinessRegistrationNum 
						Where CI_MT.RecodingState = 1 and  UCI_MT.RecodingState = 1 and UCI_MT.UnitCostDistinction = '외주단가' and II_MT.RecodingState = 1 
						and II_MT.ItemNum Like @ItemNum and II_MT.ItemDrawNum Like @ItemDrawNum and II_MT.ItemName Like @ItemName and CI_MT.CompanyName Like @CompanyName and CI_MT.BusinessRegistrationNum = @BusinessRegistrationNum
						order by II_MT.ItemNum ";
				Cmd.Parameters.Add("@CompanyName","%"+CSC1.Company+"%");
				Cmd.Parameters.Add("@BusinessRegistrationNum",CSC1.BusinessRegistrationNum);
			}
			...
			adp.Fill(ds,StartRecord, DataGrid1.PageSize, "OutSideOrder");

			adp.Fill(ds);
			DataGrid1.DataSource = ds;
			DataGrid1.DataMember = "OutSideOrder";
			DataGrid1.DataBind();
		}
			...
			foreach(DataGridItem item in DataGrid1.Items)
			{
				...
				if(cb.Checked)
				{
					...
					try
					{
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('외주단가정보', "+ item.Cells[1].Text +", 'T_Materials', "+ MaterialsIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('외주단가정보', "+ item.Cells[1].Text +", 'T_Product', "+ ProductIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
						...
						{
							Cmd.CommandText = "INSERT CR_T VALUES('외주단가정보', "+ item.Cells[1].Text +", 'T_Development', "+ DevelopmentIndex +")";
							Cmd.ExecuteNonQuery();
							trans.Commit();
		...
		private void btSearch_Click(object sender, System.EventArgs e)
		{
			...
			if(CSC1.BusinessRegistrationNum.Trim() == "")
			{
				Cmd.CommandText = @"SELECT  Count(UnitCostInfoIndex)
						FROM PUC_MT PUC_MT_1 
						INNER JOIN  UCI_MT 
						ON PUC_MT_1.SmallClassificationCode  = UCI_MT.BeginProcessCode 
						join PUC_MT PUC_MT_2 
