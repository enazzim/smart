...
{
	/// <summary>
	/// GnAdjustment에 대한 요약 설명입니다.
	/// </summary>
	public class GnAdjustment : System.Web.UI.Page
	{
		...
		private string CGeneralDataIndex = "";		
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// 현재 클릭된 글의 CGeneralDataIndex컬럼값을 Request개체로 얻어서 그 값을 저장한다.
			CGeneralDataIndex = Request.QueryString["CGeneralDataIndex"];
			// CGeneralDataIndex값이 null이면 사용자가 GeneralData.aspx 페이지에서 특정 제목을 클릭하여 GnAdjustment.aspx 페이지로
			// 이동해 온 것이 아니라, 사용자가 브라우저에 직접적으로 GnAdjustment.aspx 페이지를 요청하여 접근한 것이므로
			// Response.Redirect 메서드를 사용하여 GeneralData.aspx 페이지로 이동시킨다.
			if(CGeneralDataIndex == null)
				Response.Redirect("GeneralData.aspx");

			if(Request.Cookies["CGD_T"] != null)
				updateHits = Request.Cookies["CGD_T"]["UpdateHits"];
		...
		private void Listing()
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT CRelationShipIndex, StandardinfoTableID, SITIndex, case CommunityDataTableID when 'CGD_T' then '일반자료실' end as CommunityDataTableID , CDTIndex FROM CR_T WHERE CommunityDataTableID='CGD_T' AND CDTIndex=@CGeneralDataIndex ORDER BY CRelationShipIndex DESC";
			Cmd.Parameters.Add("@CGeneralDataIndex", SqlDbType.Int);			
			Cmd.Parameters["@CGeneralDataIndex"].Value = CGeneralDataIndex;
		...
		private void LoadData()
		{
			...
			if(updateHits == "0")
			{
				Cmd.CommandText = "UPDATE CGD_T SET Hits = Hits + 1 WHERE CGeneralDataIndex = @CGeneralDataIndex";				
				Cmd.ExecuteNonQuery();
				Response.Cookies["CGD_T"]["UpdateHits"] = "1";
			}

			Cmd.CommandText = "SELECT * FROM CGD_T WHERE CGeneralDataIndex = @CGeneralDataIndex";
			...
			if(reader.Read())
			{
				...
				HyperLink1.NavigateUrl = "GnDownload.aspx?CGeneralDataIndex=" + Request.QueryString["CGeneralDataIndex"] + "&filename=AppendFile";
			...
			int Pre = 0, Next = 0;
			SqlCommand comm = new SqlCommand("SELECT TOP 1 CGeneralDataIndex FROM CGD_T WHERE CGeneralDataIndex < @CGeneralDataIndex ORDER BY CGeneralDataIndex DESC", conn);
			comm.Parameters.Add("@CGeneralDataIndex", SqlDbType.Int).Value = Request.QueryString["CGeneralDataIndex"];
			...
			comm = new SqlCommand("SELECT TOP 1 CGeneralDataIndex FROM CGD_T WHERE CGeneralDataIndex > @CGeneralDataIndex ORDER BY CGeneralDataIndex ASC", conn);
			comm.Parameters.Add("@CGeneralDataIndex", SqlDbType.Int).Value = Request.QueryString["CGeneralDataIndex"];
			if(comm.ExecuteScalar() == null)
			{
			...
			hlnkPreContent.NavigateUrl = "GnAdjustment.aspx?CGeneralDataIndex=" + Pre.ToString();
			hlnkNextContent.NavigateUrl = "GnAdjustment.aspx?CGeneralDataIndex=" + Next.ToString();
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				Cmd.CommandText = "DELETE FROM CGD_T WHERE CGeneralDataIndex = @CGeneralDataIndex";							
				Cmd.Parameters.Add("@CGeneralDataIndex", SqlDbType.Int);
				Cmd.Parameters["@CGeneralDataIndex"].Value = CGeneralDataIndex;
				Cmd.ExecuteNonQuery();
				
				Cmd.CommandText = "DELETE FROM CR_T WHERE CDTIndex = @CGeneralDataIndex";
				Cmd.ExecuteNonQuery();
				trans.Commit();
			}
			catch(Exception ex)
		...
		private void btnEdit_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if(File1.Value == "")
					Cmd.CommandText = "UPDATE CGD_T SET Title=@Title, Category = @Category, Contents=@Contents, RegistrationDate=@RegistrationDate WHERE CGeneralDataIndex=@CGeneralDataIndex";
				else
					Cmd.CommandText = "UPDATE CGD_T SET Title=@Title, Category = @Category, Contents=@Contents, AppendFile=@AppendFile, FileName=@FileName, FileSize=@FileSize, FileType=@FileType, RegistrationDate=@RegistrationDate WHERE CGeneralDataIndex=@CGeneralDataIndex";
				...
				Cmd.Parameters.Add("@CGeneralDataIndex", SqlDbType.Int).Value = CGeneralDataIndex;
		...
		{
			Response.Write("<script>window.open('Popup/CompanyInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"','CompanyInfo', 'resizable=1, scrollbars=1, width=820, height=600');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/ItemInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"','ItemInfo', 'resizable=1, scrollbars=1, width=760, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/ItemOrganizationInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"','ItemOrganizationInfo', 'resizable=1, scrollbars=1, width=760, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/EquipmentInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"','EquipmentInfo', 'resizable=1, scrollbars=1, width=525, height=400');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/WCInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"','WCInfo', 'resizable=1, scrollbars=1, width=680, height=300');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/ProcessSequenceInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"','ProcessSequenceInfo', 'resizable=1, scrollbars=1, width=760, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/WorkStandardInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"','WorkStandardInfo', 'resizable=1, scrollbars=1, width=760, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/SellUnitCostInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"', 'SellUnitCostInfo', 'resizable=1, scrollbars=1, width=790, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/OutSideOrderUnitCostInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"', 'OutSideOrderUnitCostInfo', 'resizable=1, scrollbars=1, width=790, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/BuyUnitCostInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"', 'BuyUnitCostInfo', 'resizable=1, scrollbars=1, width=790, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/UserInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"', 'UserInfo', 'resizable=1, scrollbars=1, width=705, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/PublicUseCode.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"', 'PublicUseCode', 'resizable=1, scrollbars=1, width=760, height=800');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/BusinessPlanInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"', 'BusinessPlanInfo', 'resizable=1, scrollbars=1, width=970, height=500');</script>");		
		}
		...
		{
			Response.Write("<script>window.open('Popup/ExecutionPlanInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"', 'ExecutionPlanInfo', 'resizable=1, scrollbars=1, width=970, height=500');</script>");		
		}
		...
		{
			Response.Write("<script>window.open('Popup/MonthlyClosingInfo.aspx?CGeneralDataIndex="+CGeneralDataIndex.ToString()+"', 'MonthlyClosingInfo', 'resizable=1, scrollbars=1, width=580, height=300');</script>");		
		}
		...
		private void DataGrid1_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{				
				...
				Cmd.ExecuteNonQuery();
                trans.Commit();								
				Response.Write("<script>alert('삭제 되었습니다.'); location.href='GnAdjustment.aspx?CGeneralDataIndex=" + CGeneralDataIndex + "';; </script>");
			}
			catch(Exception ex)
			{
				Response.Write(ex.Message);
