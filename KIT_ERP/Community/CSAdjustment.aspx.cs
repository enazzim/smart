...
{
	/// <summary>
	/// CSAdjustment에 대한 요약 설명입니다.
	/// </summary>
	public class CSAdjustment : System.Web.UI.Page
	{
		...
		private string CCompanyStandardIndex = "";		
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// CCompanyStandardIndex값이 null이면 사용자가 CompanyStandard.aspx 페이지에서 특정 제목을 클릭하여 CSAdjustment.aspx 페이지로
			// 이동해 온 것이 아니라, 사용자가 브라우저에 직접적으로 CSAdjustment.aspx 페이지를 요청하여 접근한 것이므로
			// Response.Redirect 메서드를 사용하여 CompanyStandard.aspx 페이지로 이동시킨다.
			if(CCompanyStandardIndex == null)
				Response.Redirect("CompanyStandard.aspx");

			if(Request.Cookies["CCS_T"] != null)
				updateHits = Request.Cookies["CCS_T"]["UpdateHits"];
		...
		private void LoadCategory()
		{
			...
			Cmd.Connection = Con;				
			Con.Open();
			Cmd.CommandText = "SELECT Distinct Category FROM CCG_T WHERE Classify = 4";
		...
		private void Listing()
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT CRelationShipIndex, StandardinfoTableID, SITIndex, case CommunityDataTableID when 'CCS_T' then '사내표준현황' end as CommunityDataTableID , CDTIndex FROM CR_T WHERE CommunityDataTableID='CCS_T' AND CDTIndex=@CCompanyStandardIndex ORDER BY CRelationShipIndex DESC";
			Cmd.Parameters.Add("@CCompanyStandardIndex", SqlDbType.Int);			
			Cmd.Parameters["@CCompanyStandardIndex"].Value = CCompanyStandardIndex;
		...
		private void LoadData()
		{
			...
			if(updateHits == "0")
			{
				Cmd.CommandText = "UPDATE CCS_T SET Hits = Hits + 1 WHERE CCompanyStandardIndex = @CCompanyStandardIndex";				
				Cmd.ExecuteNonQuery();
				Response.Cookies["CCS_T"]["UpdateHits"] = "1";
			}

			Cmd.CommandText = "SELECT CCompanyStandardIndex, Title, Category, Contents, AppendFile, FileName, FileSize, FileType, RegistrationPerson, Convert(varchar, CCS_T.RegistrationDate, 111)RegistrationDate, Hits FROM CCS_T WHERE CCompanyStandardIndex = @CCompanyStandardIndex";
			...
			if(reader.Read())
			{
				...
				HyperLink1.NavigateUrl = "CSDownload.aspx?CCompanyStandardIndex=" + Request.QueryString["CCompanyStandardIndex"] + "&filename=AppendFile";
				
				if(reader["FileSize"].ToString() != "0")
					Label7.Text = "(" + reader["FileSize"].ToString() + ")";
				else
					Label7.Text = "<font color='red'><b>첨부된 파일없음</b></font>";
			}
			...
			int Pre = 0, Next = 0;
			SqlCommand comm = new SqlCommand("SELECT TOP 1 CCompanyStandardIndex FROM CCS_T WHERE CCompanyStandardIndex < @CCompanyStandardIndex ORDER BY CCompanyStandardIndex DESC", conn);
			comm.Parameters.Add("@CCompanyStandardIndex", SqlDbType.Int).Value = Request.QueryString["CCompanyStandardIndex"];
			...
			comm = new SqlCommand("SELECT TOP 1 CCompanyStandardIndex FROM CCS_T WHERE CCompanyStandardIndex > @CCompanyStandardIndex ORDER BY CCompanyStandardIndex ASC", conn);
			comm.Parameters.Add("@CCompanyStandardIndex", SqlDbType.Int).Value = Request.QueryString["CCompanyStandardIndex"];
			if(comm.ExecuteScalar() == null)
			{
			...
			hlnkPreContent.NavigateUrl = "CSAdjustment.aspx?CCompanyStandardIndex=" + Pre.ToString();
			hlnkNextContent.NavigateUrl = "CSAdjustment.aspx?CCompanyStandardIndex=" + Next.ToString();
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				Cmd.CommandText = "DELETE FROM CCS_T WHERE CCompanyStandardIndex = @CCompanyStandardIndex";							
				Cmd.Parameters.Add("@CCompanyStandardIndex", SqlDbType.Int);
				Cmd.Parameters["@CCompanyStandardIndex"].Value = CCompanyStandardIndex;
				Cmd.ExecuteNonQuery();
				
				Cmd.CommandText = "DELETE FROM CR_T WHERE CDTIndex = @CCompanyStandardIndex";
				Cmd.ExecuteNonQuery();
				trans.Commit();
			}
			catch(Exception ex)
		...
		private void btnEdit_Click(object sender, System.EventArgs e)
		{
			...
			{
				if(File1.Value == "")
					Cmd.CommandText = "UPDATE CCS_T SET Title=@Title,  Category = @Category, Contents=@Contents, RegistrationDate=@RegistrationDate WHERE CCompanyStandardIndex=@CCompanyStandardIndex";
				else
					Cmd.CommandText = "UPDATE CCS_T SET Title=@Title,  Category = @Category, Contents=@Contents, AppendFile=@AppendFile, FileName=@FileName, FileSize=@FileSize, FileType=@FileType, RegistrationDate=@RegistrationDate WHERE CCompanyStandardIndex=@CCompanyStandardIndex";
				...
				Cmd.Parameters.Add("@CCompanyStandardIndex", SqlDbType.Int).Value = CCompanyStandardIndex;
		...
		{
			Response.Write("<script>window.open('Popup/CompanyInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"','CompanyInfo', 'resizable=1, scrollbars=1, width=850, height=500');</script>");
		}

		// 품목정보 팝업창
		private void btnItemInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/ItemInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"','ItemInfo', 'resizable=1, scrollbars=1, width=800, height=500');</script>");		
		}

		// 품목구성정보 팝업창
		private void btnItemOrganizationInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/ItemOrganizationInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"','ItemOrganizationInfo', 'resizable=1, scrollbars=1, width=800, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/EquipmentInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"','EquipmentInfo', 'resizable=1, scrollbars=1, width=550, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/WCInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"','WCInfo', 'resizable=1, scrollbars=1, width=710, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/ProcessSequenceInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"','ProcessSequenceInfo', 'resizable=1, scrollbars=1, width=800, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/WorkStandardInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"','WorkStandardInfo', 'resizable=1, scrollbars=1, width=800, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/SellUnitCostInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"', 'SellUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/OutSideOrderUnitCostInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"', 'OutSideOrderUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/BuyUnitCostInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"', 'BuyUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/UserInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"', 'UserInfo', 'resizable=1, scrollbars=1, width=730, height=500');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/PublicUseCode.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"', 'PublicUseCode', 'resizable=1, scrollbars=1, width=790, height=800');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/BusinessPlanInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"', 'BusinessPlanInfo', 'resizable=1, scrollbars=1, width=940, height=500');</script>");		
		}
		...
		{
			Response.Write("<script>window.open('Popup/ExecutionPlanInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"', 'ExecutionPlanInfo', 'resizable=1, scrollbars=1, width=940, height=500');</script>");		
		}
		...
		{
			Response.Write("<script>window.open('Popup/MonthlyClosingInfo.aspx?CCompanyStandardIndex="+CCompanyStandardIndex.ToString()+"', 'MonthlyClosingInfo', 'resizable=1, scrollbars=1, width=600, height=300');</script>");		
		}
		...
		private void DataGrid1_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{				
				...
				Cmd.ExecuteNonQuery();
				trans.Commit();								
				Response.Write("<script>alert('삭제 되었습니다.'); location.href='CSAdjustment.aspx?CCompanyStandardIndex=" + CCompanyStandardIndex + "';; </script>");
			}
			catch(Exception ex)
			{
				Response.Write(ex.Message);
