...
{
	/// <summary>
	/// PipeList에 대한 요약 설명입니다.
	/// </summary>
	public class PipeList : System.Web.UI.Page
	{
		...
		private DataSet Search()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = @"Select  seq, thread, depth, writer, title, Convert(varchar, transdate, 111) as transdate FROM T_Pipe ORDER BY thread desc";			
		...
		{
			Response.Redirect("PipeWrite.aspx");
		}
		...
		private void DataGrid1_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			if(e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				else
				{
					...
					SqlCommand Cmd = new SqlCommand();
					Cmd.Connection = Con;
					Cmd.CommandText = "Select Count(ID) From Relation_T Where ReadDate is null and [No] = @no and Division='파이프'";
		...
		private bool Exist(string item)
		{
			...
			comm.CommandText = "Select count(ID) From Relation_T where No = @no  and ReadDate is not null and Division = '파이프'";
