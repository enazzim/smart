...
{
	/// <summary>
	/// PresidentNoticeContent에 대한 요약 설명입니다.
	/// </summary>
	public class PresidentNoticeContent : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!IsPostBack)
			{
				...
				if(seq == string.Empty)
				{
					Response.Redirect("PresidentNoticeList.aspx");
					Response.End();
				}
		...
		private void LoadData()
		{
			...
			if(reader.Read())
			{
				...
				HyperLink1.NavigateUrl = "Down.aspx?seq=" + Request.QueryString["seq"] + "&Division=PresidentNotice&filename=AppendFile";
			...
			comm.CommandText = "SELECT TOP 1 seq FROM T_PresidentNotice WHERE seq < @seq ORDER BY seq DESC";
			...
			comm = new SqlCommand("SELECT TOP 1 seq FROM T_PresidentNotice WHERE seq > @seq ORDER BY seq ASC", conn);
			comm.Parameters.Add("@seq", SqlDbType.Int).Value = Request.QueryString["seq"];
			if(comm.ExecuteScalar() == null)
			{
			...
			hlnkPreContent.NavigateUrl = "PresidentNoticeContent.aspx?seq=" + Pre.ToString();
			hlnkNextContent.NavigateUrl = "PresidentNoticeContent.aspx?seq=" + Next.ToString();
			...
			//필독자 바인딩
			comm.CommandText = @"SELECT Name + '(' + R.ID + ')'  as Name ,R.ID FROM UI_MT inner join Relation_T as R on UI_MT.ID = R.ID WHERE (RecodingState = 1) and Division='대표공지' and NO = @No ORDER BY Name";
			...
			comm.CommandText = "Select count(ID) From Relation_T where Division='대표공지' and NO = @No and [ID] = @id and ReadDate is null";
			comm.Parameters.Add("@No",seq);
			...
			{
				//
				comm.CommandText = "Update Relation_T Set ReadDate = @date where Division='대표공지' and NO = @No and [ID] = @id";
				comm.Parameters.Add("@No",seq);
				comm.Parameters.Add("@id",Session["ID"].ToString());
			...
			comm.CommandText = "SELECT Name + '(' + R.ID + ')'  as Name, R.ReadDate FROM UI_MT inner join Relation_T as R on UI_MT.ID = R.ID WHERE (RecodingState = 1) and Division='대표공지' and NO = @No and ReadDate is not null ORDER BY ReadDate, Name";
			comm.Parameters.Add("@No",seq);
		...
		private void btnReply_Click(object sender, System.EventArgs e)
		{
			...
			Response.Redirect("PresidentNoticeWrite.aspx?thread=" + thread + "&depth=" + depth + "&seq=" + seq);
		}

		private void btnEdit_Click(object sender, System.EventArgs e)
		{
			Response.Redirect("PresidentNoticeEdit.aspx?seq=" + ViewState["seq"].ToString());   
		}
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				Cmd.CommandText = "DELETE FROM T_PresidentNotice WHERE seq = @seq";							
				Cmd.Parameters.Add("@seq", SqlDbType.Int);
				Cmd.Parameters["@seq"].Value = seq;
				Cmd.ExecuteNonQuery();

				Cmd.CommandText = "Delete From Relation_T Where [No] = @seq and Division = '대표공지'";
				Cmd.ExecuteNonQuery();
