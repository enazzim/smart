...
namespace KIT_ERP.Community
{
	...
	public class FreeBoard : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Response.Cookies["CFB_T"]["UpdateHits"] = "0";
			// CFB_T라는 쿠키값이 존재하면 pageNumber값을 iPage변수에 저장하고
			if(Request.Cookies["CFB_T"] != null)
			{
				string pageNumber = Request.Cookies["CFB_T"]["PageNumber"];				
				if(pageNumber != null)
			...
			if(!IsPostBack)
			{
				//Response.Write("<script language='javascript'>document.parentWindow.parent.ContentsTitle.location = '../ContentsTitle.aspx?title=∵ 커뮤니티 > 자유게시판';</script>");
		...
		private void Listing()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT TOP 15 CFreeBoardIndex, Title, Contents, RegistrationPerson, Convert(varchar, CFB_T.RegistrationDate, 111)RegistrationDate, Hits, Depth FROM CFB_T WHERE CFreeBoardIndex NOT IN (SELECT TOP "+iPage*15+" CFreeBoardIndex FROM CFB_T ORDER BY CFreeBoardIndex desc) ORDER BY CFreeBoardIndex desc";
		...
		// CFB_T 테이블의 총 레코드 수 구하기
		private int GetTotalRecordCount()
		{
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string strSql = "SELECT Count(*) FROM CFB_T";
		...
		private void DataGrid1_PageIndexChanged(object source, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			Response.Cookies["CFB_T"]["PageNumber"] = iPage.ToString();
			Listing();
		}
