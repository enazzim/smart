...
{
	/// <summary>
	/// PresidentNoticeList_aspx에 대한 요약 설명입니다.
	/// </summary>
	public class PresidentNoticeList_aspx : System.Web.UI.Page
	{
		...
		private DataSet Search()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = @"Select  seq, thread, depth, writer, title, Convert(varchar, transdate, 111) as transdate FROM T_PresidentNotice ORDER BY thread desc";			
		...
		{
			Response.Redirect("PresidentNoticeWrite.aspx");
		}
		...
		private bool Exist(string item)
		{
			...
			comm.CommandText = "Select count(ID) From Relation_T where No = @no  and ReadDate is not null and Division = '대표공지'";
