...
namespace KIT_ERP.Community
{
	...
	public class ProductList : System.Web.UI.Page
	{
		...
		private DataSet Search()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = @"Select  seq, thread, depth, writer, title, Convert(varchar, transdate, 111) as transdate FROM T_Product ORDER BY thread desc";			
		...
		{
			Response.Redirect("ProductWrite.aspx");
		}
		...
		private bool Exist(string item)
		{
			...
			comm.CommandText = "Select count(ID) From Relation_T where No = @no  and ReadDate is not null and Division = '생산품질'";
