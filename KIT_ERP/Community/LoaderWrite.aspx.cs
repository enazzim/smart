...
{
	/// <summary>
	/// LoaderWrite에 대한 요약 설명입니다.
	/// </summary>
	public class LoaderWrite : System.Web.UI.Page
	{
		...
		{    
			...
			// sqlInsertCommand1
			// 
			this.sqlInsertCommand1.CommandText = "INSERT INTO UI_MT(ID) VALUES (@ID); SELECT Name + \'(\' + ID + \')\' AS Name, ID, Use" +
				"rInfoIndex FROM UI_MT WHERE (UserInfoIndex = @@IDENTITY) ORDER BY Name";
			...
			// sqlSelectCommand1
			// 
			this.sqlSelectCommand1.CommandText = "SELECT Name + \'(\' + ID + \')\' AS Name, ID, UserInfoIndex FROM UI_MT WHERE (Recodin" +
				"gState = 1) ORDER BY Name";
		...
		private void btnApply_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				Cmd.Parameters.Add("@Division","로더생산현황");
		...
		private int Max(SqlConnection conn, SqlTransaction tr)
		{
			...
			comm.Transaction = tr;
			comm.CommandText = "Select Max(seq) From T_Loader";
