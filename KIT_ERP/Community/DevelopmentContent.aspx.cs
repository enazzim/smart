...
{
	/// <summary>
	/// DevelopmentContent에 대한 요약 설명입니다.
	/// </summary>
	public class DevelopmentContent : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			DevelopmentIndex = Request.QueryString["seq"];
		...
		private void Listing()
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT CRelationShipIndex, StandardinfoTableID, SITIndex, case CommunityDataTableID when 'T_Development' then '개발실' end as CommunityDataTableID , CDTIndex FROM CR_T WHERE CommunityDataTableID='T_Development' AND CDTIndex=@DevelopmentIndex ORDER BY CRelationShipIndex DESC";
			Cmd.Parameters.Add("@DevelopmentIndex", SqlDbType.Int);			
			Cmd.Parameters["@DevelopmentIndex"].Value = DevelopmentIndex;
		...
		private void LoadData()
		{
			...
			if(reader.Read())
			{
				...
				HyperLink1.NavigateUrl = "Down.aspx?seq=" + Request.QueryString["seq"] + "&Division=Development&filename=AppendFile";
			...
			comm.CommandText = "SELECT TOP 1 seq FROM T_Development WHERE seq < @seq ORDER BY seq DESC";
			...
			comm = new SqlCommand("SELECT TOP 1 seq FROM T_Development WHERE seq > @seq ORDER BY seq ASC", conn);
			comm.Parameters.Add("@seq", SqlDbType.Int).Value = Request.QueryString["seq"];
			if(comm.ExecuteScalar() == null)
			{
			...
			hlnkPreContent.NavigateUrl = "DevelopmentContent.aspx?seq=" + Pre.ToString();
			hlnkNextContent.NavigateUrl = "DevelopmentContent.aspx?seq=" + Next.ToString();
			...
			//필독자 바인딩
			comm.CommandText = @"SELECT Name + '(' + R.ID + ')'  as Name ,R.ID FROM UI_MT inner join Relation_T as R on UI_MT.ID = R.ID WHERE (RecodingState = 1) and Division='개발실' and NO = @No ORDER BY Name";
			...
			comm.CommandText = "Select count(ID) From Relation_T where Division='개발실' and NO = @No and [ID] = @id and ReadDate is null";
			comm.Parameters.Add("@No",seq);
			...
			{
				//
				comm.CommandText = "Update Relation_T Set ReadDate = @date where Division='개발실' and NO = @No and [ID] = @id";
				comm.Parameters.Add("@No",seq);
				comm.Parameters.Add("@id",Session["ID"].ToString());
			...
			comm.CommandText = "SELECT Name + '(' + R.ID + ')'  as Name, R.ReadDate FROM UI_MT inner join Relation_T as R on UI_MT.ID = R.ID WHERE (RecodingState = 1) and Division='개발실' and NO = @No and ReadDate is not null ORDER BY ReadDate, Name";
			comm.Parameters.Add("@No",seq);
		...
		private void btnReply_Click(object sender, System.EventArgs e)
		{
			...
			Response.Redirect("DevelopmentWrite.aspx?thread=" + thread + "&depth=" + depth + "&seq=" + seq);
		}

		private void btnEdit_Click(object sender, System.EventArgs e)
		{
			Response.Redirect("DevelopmentEdit.aspx?seq=" + ViewState["seq"].ToString());   
		}
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				Cmd.CommandText = "DELETE FROM T_Development WHERE seq = @seq";							
				Cmd.Parameters.Add("@seq", SqlDbType.Int);
				Cmd.Parameters["@seq"].Value = seq;
				Cmd.ExecuteNonQuery();

				Cmd.CommandText = "Delete From Relation_T Where [No] = @seq and Division = '개발실'";
				Cmd.ExecuteNonQuery();
		...
		{
			Response.Write("<script>window.open('Popup/CompanyInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"','CompanyInfo', 'resizable=1, scrollbars=1, width=850, height=530');</script>");
		}

		// 품목정보 팝업창
		private void btnItemInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/ItemInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"','ItemInfo', 'resizable=1, scrollbars=1, width=800, height=530');</script>");		
		}

		// 품목구성정보 팝업창
		private void btnItemOrganizationInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/ItemOrganizationInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"','ItemOrganizationInfo', 'resizable=1, scrollbars=1, width=700, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/EquipmentInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"','EquipmentInfo', 'resizable=1, scrollbars=1, width=550, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/WCInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"','WCInfo', 'resizable=1, scrollbars=1, width=710, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/ProcessSequenceInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"','ProcessSequenceInfo', 'resizable=1, scrollbars=1, width=800, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/WorkStandardInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"','WorkStandardInfo', 'resizable=1, scrollbars=1, width=800, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/SaleUnitCostInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'SaleUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/OutSideOrderUnitCostInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'OutSideOrderUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/BuyUnitCostInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'BuyUnitCostInfo', 'resizable=1, scrollbars=1, width=810, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/UserInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'UserInfo', 'resizable=1, scrollbars=1, width=730, height=530');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/PublicUseCode.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'PublicUseCode', 'resizable=1, scrollbars=1, width=790, height=800');</script>");
		}
		...
		{
			Response.Write("<script>window.open('Popup/BusinessPlanInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'BusinessPlanInfo', 'resizable=1, scrollbars=1, width=940, height=530');</script>");		
		}
		...
		{
			Response.Write("<script>window.open('Popup/ExecutionPlanInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'ExecutionPlanInfo', 'resizable=1, scrollbars=1, width=940, height=530');</script>");		
		}

		//실공정정보 팝업
		private void btnRealProcessSequenceInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/RealProcessSequenceInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'RealProcessSequenceInfo', 'resizable=1, scrollbars=1, width=700, height=530');</script>");		
		}

		//실품목구성 팝업
		private void btnRealItemOrganizationInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/RealItemOrganizationInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'RealItemOrganizationInfo', 'resizable=1, scrollbars=1, width=700, height=530');</script>");		
		}
		//실 작업표준 팝업
		private void btnRealWorkStandardInfo_Click(object sender, System.EventArgs e)
		{
			Response.Write("<script>window.open('Popup/RealWorkStandardInfo.aspx?DevelopmentIndex="+DevelopmentIndex.ToString()+"', 'RealWorkStandardInfo', 'resizable=1, scrollbars=1, width=700, height=530');</script>");		
		}
		...
		private void Datagrid2_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{				
				...
				Cmd.ExecuteNonQuery();
				trans.Commit();								
				Response.Write("<script>alert('삭제 되었습니다.'); location.href='DevelopmentContent.aspx?seq=" + DevelopmentIndex + "';; </script>");
			}
			catch(Exception ex)
			{
				Response.Write(ex.Message);
