...
namespace KIT_ERP.Community
{
	...
	public class Notice : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Response.Cookies["CN_T"]["UpdateHits"] = "0";
			
			// CN_T라는 쿠키값이 존재하면 pageNumber값을 iPage변수에 저장하고
			if(Request.Cookies["CN_T"] != null)
			{
				string pageNumber = Request.Cookies["CN_T"]["PageNumber"];				
			...
			if(!IsPostBack)
			{
				//Response.Write("<script language='javascript'>document.parentWindow.parent.ContentsTitle.location = '../ContentsTitle.aspx?title=∵ 커뮤니티 > 공지사항';</script>");
		...
		private void Listing()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT TOP 15 CNoticeIndex, Title, Contents, RegistrationPerson, Convert(varchar, CN_T.RegistrationDate, 111)RegistrationDate, Hits FROM CN_T WHERE CNoticeIndex NOT IN (SELECT TOP "+iPage*15+" CNoticeIndex FROM CN_T ORDER BY CNoticeIndex DESC) ORDER BY CNoticeIndex desc";
		...
		// CN_T 테이블의 총 레코드 수 구하기
		private int GetTotalRecordCount()
		{
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string strSql = "SELECT Count(*) FROM CN_T";
		...
		{
			Response.Redirect("NoRegistration.aspx");
		}
