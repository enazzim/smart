...
{
	/// <summary>
	/// LoaderContent에 대한 요약 설명입니다.
	/// </summary>
	public class LoaderContent : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			LoaderIndex = Request.QueryString["seq"];
			...
			if(!IsPostBack)
			{
				...
				if(seq == string.Empty)
				{
					Response.Redirect("LoaderList.aspx");
					Response.End();
				}
		...
		private void Listing()
		{
			...
			SqlCommand Cmd = new SqlCommand();
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT CRelationShipIndex, StandardinfoTableID, SITIndex, case CommunityDataTableID when 'T_Loader' then '로더생산현황' end as CommunityDataTableID , CDTIndex FROM CR_T WHERE CommunityDataTableID='T_Loader' AND CDTIndex=@LoaderIndex ORDER BY CRelationShipIndex DESC";
			Cmd.Parameters.Add("@LoaderIndex", SqlDbType.Int);			
			Cmd.Parameters["@LoaderIndex"].Value = LoaderIndex;
		...
		private void LoadData()
		{
			...
			if(reader.Read())
			{
				...
				HyperLink1.NavigateUrl = "Down.aspx?seq=" + Request.QueryString["seq"] + "&Division=Loader&filename=AppendFile";
			...
			comm.CommandText = "SELECT TOP 1 seq FROM T_Loader WHERE seq < @seq ORDER BY seq DESC";
			...
			comm = new SqlCommand("SELECT TOP 1 seq FROM T_Loader WHERE seq > @seq ORDER BY seq ASC", conn);
			comm.Parameters.Add("@seq", SqlDbType.Int).Value = Request.QueryString["seq"];
			if(comm.ExecuteScalar() == null)
			{
			...
			hlnkPreContent.NavigateUrl = "LoaderContent.aspx?seq=" + Pre.ToString();
			hlnkNextContent.NavigateUrl = "LoaderContent.aspx?seq=" + Next.ToString();
			...
			//필독자 바인딩
			comm.CommandText = @"SELECT Name + '(' + R.ID + ')'  as Name ,R.ID FROM UI_MT inner join Relation_T as R on UI_MT.ID = R.ID WHERE (RecodingState = 1) and Division='로더생산현황' and NO = @No ORDER BY Name";
			...
			comm.CommandText = "Select count(ID) From Relation_T where Division='로더생산현황' and NO = @No and [ID] = @id and ReadDate is null";
			comm.Parameters.Add("@No",seq);
			...
			{
				//
				comm.CommandText = "Update Relation_T Set ReadDate = @date where Division='로더생산현황' and NO = @No and [ID] = @id";
				comm.Parameters.Add("@No",seq);
				comm.Parameters.Add("@id",Session["ID"].ToString());
			...
			comm.CommandText = "SELECT Name + '(' + R.ID + ')'  as Name, R.ReadDate FROM UI_MT inner join Relation_T as R on UI_MT.ID = R.ID WHERE (RecodingState = 1) and Division='로더생산현황' and NO = @No and ReadDate is not null ORDER BY ReadDate, Name";
			comm.Parameters.Add("@No",seq);
		...
		private void btnReply_Click(object sender, System.EventArgs e)
		{
			...
			Response.Redirect("LoaderWrite.aspx?thread=" + thread + "&depth=" + depth + "&seq=" + seq);
		}
		...
		{
			Response.Redirect("LoaderEdit.aspx?seq=" + ViewState["seq"].ToString());   
		}
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				Cmd.CommandText = "DELETE FROM T_Loader WHERE seq = @seq";							
				Cmd.Parameters.Add("@seq", SqlDbType.Int);
				Cmd.Parameters["@seq"].Value = seq;
				Cmd.ExecuteNonQuery();

				Cmd.CommandText = "Delete From Relation_T Where [No] = @seq and Division = '로더생산현황'";
				Cmd.ExecuteNonQuery();
