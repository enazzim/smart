...
{
	/// <summary>
	/// OtherInOutStorehouse에 대한 요약 설명입니다.
	/// </summary>
	public class OtherInOutStorehouse : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.DropDownList dl_Process;
		...
		protected System.Web.UI.HtmlControls.HtmlInputRadioButton rb_RadioButton1;
		protected System.Web.UI.HtmlControls.HtmlInputRadioButton rb_RadioButton2;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				...
				bt_InOutStore.Attributes.Add("onclick","return OK('선택항목을 입/출고 ');");
				...
				string str1 = "Select CompanyName, PresidentName, BusinessRegistrationNum, CompanyInfoIndex From CI_MT Where RecodingState = 1 and OutSideOrderCompany = 1 order by CompanyName";
				...
				wc_Company.DataValueField = ds1.Tables[0].Columns[2].ToString();
				wc_Company.DataBind();
				...
				string str_Reason = "Select SmallClassificationName, SmallClassificationCode from PUC_MT where SmallClassificationName != '' and RecodingState = 1 and LargeClassificationCode = '1500'" ;
		...
		private DataSet Search()
		{
			...
			if(ItemSearchControl1.hdItem.Trim() == "")
                search = new Search("OtherInOutStorehouse",ItemSearchControl1.ItemNum,ItemSearchControl1.ItemDrawNum,ItemSearchControl1.ItemName,wc_Company,dl_Store.SelectedItem.Text,int.Parse(dl_Store.SelectedItem.Value));
			else
				search = new Search("OtherInOutStorehouse",ItemSearchControl1.ItemNum, "", "",wc_Company,dl_Store.SelectedItem.Text,int.Parse(dl_Store.SelectedItem.Value));
		...
		{
			if(rb_RadioButton1.Checked == false && rb_RadioButton2.Checked == false)
			{
				Response.Write("<script language=javascript>");
				Response.Write("alert('입출고를 선택하세요!');");
				Response.Write("</script>");
			}
			...
			{
				Response.Write("<script language=javascript>");
				Response.Write("<alert('입출고할 품목을 그리드에서 선택하세요!');");
				Response.Write("</script>");
			}
			else if(dl_Reason.SelectedItem.Value == "")
			{
				RegisterStartupScript("","<script>alert('입출고 사유를 선택하세요!');</script>");
			}
			else
			{
				...
				//입출고 입력
				if(rb_RadioButton1.Checked == true)//출고0입고1
					arr.Add("0");
				else
					arr.Add("1");

				//창고명
				arr.Add(dl_Store.SelectedItem.Text);
				//영업창고번호
				arr.Add(dl_Store.SelectedItem.Value);

				//입출고 사유
				arr.Add(dl_Reason.SelectedItem.Value);//입출고사유코드
				arr.Add(dl_Reason.SelectedItem.Text);//입출고사유내용
				arr.Add(tb_Contents.Text);// 입출고사유 세부내용
				if(hdComname.Value == "null")
				{
					arr.Add("");
					arr.Add("");
				}
				...
				{
					arr.Add(hdComname.Value);//거래처명
					arr.Add(hdComnum.Value);//사업자등록번호
				}
				arr.Add(wdctDate.Text);
				arr.Add(int.Parse(lb_RowSelectIndex.Value));//그리드 Row 인덱스

				Register reg = new Register(UltraWebGrid1,arr,"OtherInOutStorehouse",Session["ID"].ToString());
				reg.RowRegistration();
