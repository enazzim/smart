...
{
	/// <summary>
	/// CompanyIntroduce에 대한 요약 설명입니다.
	/// </summary>
	public class CompanyIntroduce : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			// HBP_T라는 쿠키값이 존재하면 pageNumber값을 iPage변수에 저장하고
		...
		private void Listing()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT TOP 10 HBizProductIntroIndex, Title, Contents, RegistrationPerson, Convert(varchar, HBP_T.RegistrationDate, 111)RegistrationDate, Hits FROM HBP_T WHERE Distinction = 1 AND HBizProductIntroIndex NOT IN(SELECT TOP "+iPage*10+" HBizProductIntroIndex FROM HBP_T WHERE Distinction = 1 ORDER BY HBizProductIntroIndex desc) ORDER BY HBizProductIntroIndex desc";
		...
		// 레코드 총 수 구하기
		// HBP_T 테이블의 회사소개 항목 총 레코드 수 구하기
		private int GetTotalRecordCount()
		{
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string strSql = "SELECT Count(*) FROM HBP_T WHERE Distinction=1";
		...
		{
			Response.Redirect("CompanyIntroduceRegistration.aspx");
		}
		...
		private void DataGrid1_PageIndexChanged(object source, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			Response.Cookies["HBP_T"]["PageNumber"] = iPage.ToString();
			Listing();
		}
	}
}
