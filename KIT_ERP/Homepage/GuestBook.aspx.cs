...
namespace KIT_ERP.Homepage
{
	...
	public class GuestBook : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			Response.Cookies["HG_T"]["UpdateHits"] = "0";
			// HG_T라는 쿠키값이 존재하면 pageNumber값을 iPage변수에 저장하고
			if(Request.Cookies["HG_T"] != null)
			{
				string pageNumber = Request.Cookies["HG_T"]["PageNumber"];				
				if(pageNumber != null)
		...
		private void Listing()
		{
			...
			Cmd.Connection = Con;
			Cmd.CommandText = "SELECT TOP 10 HGuestIndex, Title, Contents, RegistrationPerson, Convert(varchar, HG_T.RegistrationDate, 111)RegistrationDate, Hits, Depth FROM HG_T WHERE HGuestIndex NOT IN (SELECT TOP "+iPage*10+" HGuestIndex FROM HG_T ORDER BY HGuestIndex desc) ORDER BY HGuestIndex desc";
		...
		// HG_T 테이블의 총 레코드 수 구하기
		private int GetTotalRecordCount()
		{
			string ConnectStr = ConfigurationSettings.AppSettings["DSN"].ToString();
			SqlConnection Con = new SqlConnection(ConnectStr);
			string strSql = "SELECT Count(*) FROM HG_T";
		...
		private void DataGrid1_PageIndexChanged(object source, System.Web.UI.WebControls.DataGridPageChangedEventArgs e)
		{
			...
			Response.Cookies["HG_T"]["PageNumber"] = iPage.ToString();
			Listing();
		}
		...
		{
			Response.Redirect("GuestBookRegistration.aspx");
		}
