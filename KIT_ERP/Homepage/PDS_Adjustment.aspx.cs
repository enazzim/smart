...
{
	/// <summary>
	/// PDS_Adjustment에 대한 요약 설명입니다.
	/// </summary>
	public class PDS_Adjustment : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			HDataIndex = Request.QueryString["HDataIndex"];

			if(HDataIndex == null)
				Response.Redirect("PDS.aspx");

			if(Request.Cookies["HBP_T"] != null)
				updateHits = Request.Cookies["HD_T"]["UpdateHits"];		
...
//				DropDownList1.DataBind();
		...
		private void LoadData()
		{
			...
			Con.Open();
			Cmd.Parameters.Add("@HDataIndex", SqlDbType.Int);
			Cmd.Parameters["@HDataIndex"].Value = HDataIndex;

			if(updateHits == "0")
			{
				Cmd.CommandText = "UPDATE HD_T SET Hits = Hits + 1 WHERE HDataIndex = @HDataIndex";				
				Cmd.ExecuteNonQuery();
				Response.Cookies["HD_T"]["UpdateHits"] = "1";
			}

			Cmd.CommandText = "SELECT * FROM HD_T WHERE HDataIndex = @HDataIndex";
			SqlDataReader reader = Cmd.ExecuteReader(CommandBehavior.CloseConnection);
			...
			if(reader.Read())
			{
				...
				HyperLink1.NavigateUrl = "PDS_Download.aspx?HDataIndex=" + Request.QueryString["HDataIndex"] + "&filename=AppendFile";
			...
			int Pre = 0, Next = 0;
			SqlCommand comm = new SqlCommand("SELECT TOP 1 HDataIndex FROM HD_T WHERE HDataIndex < @HDataIndex ORDER BY HDataIndex DESC", conn);
			comm.Parameters.Add("@HDataIndex", SqlDbType.Int).Value = Request.QueryString["HDataIndex"];
			...
			comm = new SqlCommand("SELECT TOP 1 HDataIndex FROM HD_T WHERE HDataIndex > @HDataIndex ORDER BY HDataIndex ASC", conn);
			comm.Parameters.Add("@HDataIndex", SqlDbType.Int).Value = Request.QueryString["HDataIndex"];
			if(comm.ExecuteScalar() == null)
			{
			...
			HyperLink3.NavigateUrl = "PDS_Adjustment.aspx?HDataIndex=" + Pre.ToString();
			HyperLink4.NavigateUrl = "PDS_Adjustment.aspx?HDataIndex=" + Next.ToString();
		...
		private void Button1_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				Cmd.CommandText = "DELETE FROM HD_T WHERE HDataIndex = @HDataIndex";							
				Cmd.Parameters.Add("@HDataIndex", SqlDbType.Int);
				Cmd.Parameters["@HDataIndex"].Value = HDataIndex;
				Cmd.ExecuteNonQuery();
				trans.Commit();
		...
		private void Button2_Click(object sender, System.EventArgs e)
		{
			...
			{
				if(File1.Value == "")
					//Cmd.CommandText = "UPDATE HD_T SET Title=@Title, Contents=@Contents, RegistrationDate=@RegistrationDate WHERE HBizProductIntroIndex=@HBizProductIntroIndex";
					Cmd.CommandText = "UPDATE HD_T SET Title=@Title, Contents=@Contents, RegistrationDate=@RegistrationDate, Category = @Category WHERE HDataIndex=@HDataIndex";
				else
					Cmd.CommandText = "UPDATE HD_T SET Title=@Title, Contents=@Contents, AppendFile=@AppendFile, FileName=@FileName, FileSize=@FileSize, FileType=@FileType, RegistrationDate=@RegistrationDate, Category = @Category WHERE HDataIndex=@HDataIndex";
				...
				Cmd.Parameters.Add("@HDataIndex", SqlDbType.Int).Value = HDataIndex;
