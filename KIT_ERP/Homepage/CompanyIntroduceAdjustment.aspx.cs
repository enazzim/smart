...
{
	/// <summary>
	/// CompanyIntroduceAdjustment에 대한 요약 설명입니다.
	/// </summary>
	public class CompanyIntroduceAdjustment : System.Web.UI.Page
	{
		...
		private void LoadData()
		{
			...
			int Pre = 0, Next = 0;
			SqlCommand comm = new SqlCommand("SELECT TOP 1 HBizProductIntroIndex FROM HBP_T WHERE HBizProductIntroIndex < @HBizProductIntroIndex AND Distinction = 1 ORDER BY HBizProductIntroIndex DESC", conn);
			...
			comm = new SqlCommand("SELECT TOP 1 HBizProductIntroIndex FROM HBP_T WHERE HBizProductIntroIndex > @HBizProductIntroIndex AND Distinction = 1 ORDER BY HBizProductIntroIndex ASC", conn);
			comm.Parameters.Add("@HBizProductIntroIndex", SqlDbType.Int).Value = Request.QueryString["HBizProductIntroIndex"];
			if(comm.ExecuteScalar() == null)
			{
			...
			HyperLink3.NavigateUrl = "CompanyIntroduceAdjustment.aspx?HBizProductIntroIndex=" + Pre.ToString();
			HyperLink4.NavigateUrl = "CompanyIntroduceAdjustment.aspx?HBizProductIntroIndex=" + Next.ToString();
		...
		private void Button1_Click(object sender, System.EventArgs e)
		{
			...
			{
				Con.Close();
				Response.Redirect("CompanyIntroduce.aspx");
			}
		}
		...
		private void Button2_Click(object sender, System.EventArgs e)
		{
			...
			{
				if(File1.Value == "")
					Cmd.CommandText = "UPDATE HBP_T SET Title=@Title, Contents=@Contents, RegistrationDate=@RegistrationDate WHERE HBizProductIntroIndex=@HBizProductIntroIndex";
				else
                    Cmd.CommandText = "UPDATE HBP_T SET Title=@Title, Contents=@Contents, AppendFile=@AppendFile, FileName=@FileName, FileSize=@FileSize, FileType=@FileType, RegistrationDate=@RegistrationDate WHERE HBizProductIntroIndex=@HBizProductIntroIndex";
