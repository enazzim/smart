...
{
	/// <summary>
	/// WCStandardInfo에 대한 요약 설명입니다.
	/// </summary>
	public class WorkStandardInfo : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.Label lb_Process;
		protected System.Web.UI.WebControls.Label lb_ProcessSequence;
		protected System.Web.UI.WebControls.Label Label1;
		protected System.Web.UI.WebControls.Button bt_WorkStandard;
		protected System.Web.UI.WebControls.Label lb_PriorityOrder;
		protected System.Web.UI.WebControls.Label lb_MainWorker;
		protected System.Web.UI.WebControls.DropDownList dl_MainWorker;
		protected System.Web.UI.WebControls.DropDownList dl_WCName;
		protected System.Web.UI.WebControls.Label lb_UseTool1;
		protected System.Web.UI.WebControls.Label lb_UseTool2;
		protected System.Web.UI.WebControls.Label lb_UseTool3;
		protected System.Web.UI.WebControls.Label lb_UseJig1;
		protected System.Web.UI.WebControls.Label lb_UseJig2;
		protected System.Web.UI.WebControls.Label lb_UseJig3;
		protected System.Web.UI.WebControls.Label lb_SetupTime;
		protected System.Web.UI.WebControls.Label lb_RealProcessingTime;
		protected System.Web.UI.WebControls.Label lb_SpaceTime;
		protected System.Web.UI.WebControls.Label lb_StandardTime;
		...
		protected System.Web.UI.WebControls.Label lb_LotSize;
		protected System.Web.UI.WebControls.Label lb_WaitTime;
		...
		{    
			...
			this.bt_WorkStandard.Click += new System.EventHandler(this.bt_WorkStandard_Click);
		...
		private void page_Load()
		{
			...
			string str_MainWorker = "Select Name, ID from UI_MT where RecodingState = 1 order by [Name]";
		...
		private void bt_Registration_Click(object sender, System.EventArgs e)
		{
			...
			MasterInfoRecordRUD rud = new MasterInfoRecordRUD("WorkStandardInfo","Registration","WSI_MT",(int)PageName.WorkStandardInfo,Session["ID"].ToString(),arr,"낱개입력");
			rud.ActivateRUD();
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void bt_WorkStandard_Click(object sender, System.EventArgs e)
		{
			//if(lb_ItemIndex.Value == "0")
			//lb_ProcessCode.Value = lb_ProcessCode1.Value;
			if(ItemSearchControl1.ItemNum == "")
			{
				RegisterStartupScript("","<script>alert('공정을 선택해 주세요');</script>");
			}
			else
				Data_Bind();
		}
		...
		private void Data_Bind()
		{
			...
			View view = new View(hdItemNum.Value.ToString(),hdProcessNum.Value.ToString(),"WorkStandardInfo");
		...
		private void bt_Update_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				MasterInfoRecordRUD rud = new MasterInfoRecordRUD("WorkStandardInfo","Update","WSI_MT",(int)PageName.WorkStandardInfo,Session["ID"].ToString(),arr,"낱개입력");
				rud.ActivateRUD();
		...
		private void bt_Delete_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				MasterInfoRecordRUD rud = new MasterInfoRecordRUD("WorkStandardInfo","Delete","WSI_MT",(int)PageName.WorkStandardInfo,Session["ID"].ToString(),arr,"낱개입력");
				rud.ActivateRUD();
		...
		private void bt_Clear_Click(object sender, System.EventArgs e)
		{
			...
			View view = new View("","0","WorkStandardInfo");
			UltraWebGrid1.DataSource = view.ItemView();
			UltraWebGrid1.DataBind();
		...
		{
			RegisterStartupScript("","<script>window.open('./Popup/WorkStandard.aspx','WorkStandard'," + "'width=830" + "," + "height=500px');</script>");
		}
		...
		private void pop_Click(object sender, System.EventArgs e)
		{
			...
			string formID = "WorkStandardInfo" ;
			string txtID = "ProcessSequenceInfoIndex";
			
			RegisterStartupScript("","<script>window.open('./Popup/Process.aspx?formID=" + formID + "&txtID=" + txtID +  "&txt=" + txt + "','Item'," + "'width=850" + "," + "height=400px');</script>");
		}
		...
		private void UltraWebGrid1_DblClick(object sender, Infragistics.WebUI.UltraWebGrid.ClickEventArgs e)
		{
			...
			string str = "Select * From WSI_MT Where WorkStandardInfoIndex = @index";
			...
			{
				if( dl_WCName.Items[i].Text == ds.Tables[0].Rows[0]["WCName"].ToString())
				{
					dl_WCName.SelectedIndex = i;				
				}
			}

			tb_PriorityOrder.Text = ds.Tables[0].Rows[0]["PriorityOrder"].ToString();

			for (int i=0 ; i < dl_MainWorker.Items.Count ; i++)
			{
				if( dl_MainWorker.Items[i].Value == ds.Tables[0].Rows[0]["MainWorkerID"].ToString())
				{
					dl_MainWorker.SelectedIndex = i;				
				}
			}

			tb_Cavity.Text = ds.Tables[0].Rows[0]["Cavity"].ToString();

			for (int i=0 ; i < dl_UseTool1.Items.Count ; i++)
			{
				if( dl_UseTool1.Items[i].Text == ds.Tables[0].Rows[0]["ToolName1"].ToString())
				{
					dl_UseTool1.SelectedIndex = i;				
				}
			}
			for (int i=0 ; i < dl_UseTool2.Items.Count ; i++)
			{
				if( dl_UseTool2.Items[i].Text == ds.Tables[0].Rows[0]["ToolName2"].ToString())
				{
					dl_UseTool2.SelectedIndex = i;				
				}
			}
			for (int i=0 ; i < dl_UseTool1.Items.Count ; i++)
			{
				if( dl_UseTool3.Items[i].Text == ds.Tables[0].Rows[0]["ToolName3"].ToString())
				{
					dl_UseTool3.SelectedIndex = i;				
				}
			}
				
			for (int i=0 ; i < dl_UseJig1.Items.Count ; i++)
			{
				if( dl_UseJig1.Items[i].Text == ds.Tables[0].Rows[0]["JigName1"].ToString())
				{
					dl_UseJig1.SelectedIndex = i;				
				}
			}
			for (int i=0 ; i < dl_UseJig2.Items.Count ; i++)
			{
				if( dl_UseJig2.Items[i].Text == ds.Tables[0].Rows[0]["JigName2"].ToString())
				{
					dl_UseJig2.SelectedIndex = i;				
				}
			}
			for (int i=0 ; i < dl_UseJig3.Items.Count ; i++)
			{
				if( dl_UseJig3.Items[i].Text == ds.Tables[0].Rows[0]["JigName3"].ToString())
				{
					dl_UseJig3.SelectedIndex = i;				
				}
			}

			tb_SetupTime.Text = ds.Tables[0].Rows[0]["SetupTime"].ToString();
			tb_StandardTime.Text = ds.Tables[0].Rows[0]["StandardTime"].ToString();
			tb_RealProcessingTime.Text = ds.Tables[0].Rows[0]["RealProcessingTime"].ToString();
			tb_SpaceTime.Text = ds.Tables[0].Rows[0]["SpaceTime"].ToString();
			tb_WaitTime.Text = ds.Tables[0].Rows[0]["WaitTime"].ToString();
			tb_LotSize.Text = ds.Tables[0].Rows[0]["LotSize"].ToString();
			lb_Index.Value = ds.Tables[0].Rows[0]["WorkStandardInfoIndex"].ToString(); //작업정보 인덱스
		...
		private void LinkButton2_Click(object sender, System.EventArgs e)
		{
			...
			string str = @"Select PSI_MT.ItemNum, II_MT.ItemDrawNum, II_MT.ItemName,II_MT.PropertyClassification, PSI_MT.ProcessSequenceNum, PSI_MT.ProcessCode, PUC_MT.SmallClassificationName
				From PSI_MT inner join II_MT on PSI_MT.ItemNum = II_MT.ItemNum 
				inner join PUC_MT on PSI_MT.ProcessCode = PUC_MT.SmallClassificationCode 
				Where PSI_MT.RecodingState =  1 and  II_MT.RecodingState = 1 and PSI_MT.ItemNum = @itemnum and WorkDistinction != '외주' order by PSI_MT.ProcessSequenceNum";
			SqlCommand comm = new SqlCommand(str,conn);
		...
		{
			RegisterStartupScript("","<script language='JavaScript'>window.open('CopyWorkStandardInfo.aspx','CopyWorkStandardInfo','width=810,height=100px,resizable=yes, status=yes');</script>");
		}
