<%@ Page language="c#" Codebehind="RealWorkStandardInfo.aspx.cs" AutoEventWireup="false" Inherits="KIT_ERP.BasisInformation.RealWorkStandardInfo" %>
...
<HTML>
	<HEAD>
		<title>RealWCStandardInfo</title>
		...
		function UltraWebGrid1_DblClickHandler(gridName, cellId)
		{
			...
			//WC명
			for(var i=0;i<document.RealWorkStandardInfo.dl_WCName.options.length;i++)
			{
				if(document.RealWorkStandardInfo.dl_WCName.options[i].value == row.getCellFromKey("WCName").getValue())
				{
					document.RealWorkStandardInfo.dl_WCName.options[i].selected=true;
				}
			}
			
			//주작업자
			for(var i=0;i<document.RealWorkStandardInfo.dl_MainWorker.options.length;i++)
			{
				if(document.RealWorkStandardInfo.dl_MainWorker.options[i].value == row.getCellFromKey("MainWorkerID").getValue())
				{
					document.RealWorkStandardInfo.dl_MainWorker.options[i].selected=true;
					
				}
			}
			
			//사용공구1
			for(var i=0;i<document.RealWorkStandardInfo.dl_UseTool1.options.length;i++)
			{
				if(document.RealWorkStandardInfo.dl_UseTool1.options[i].innerText == row.getCellFromKey("ToolName1").getValue())
				{
					document.RealWorkStandardInfo.dl_UseTool1.options[i].selected=true;
					
				}
			}
			
			//사용치구1
			for(var i=0;i<document.RealWorkStandardInfo.dl_UseJig1.options.length;i++)
			{
				if(document.RealWorkStandardInfo.dl_UseJig1.options[i].innerText == row.getCellFromKey("JigName1").getValue())
				{
					document.RealWorkStandardInfo.dl_UseJig1.options[i].selected=true;
					
				}
			}
			
			//사용공구2
			for(var i=0;i<document.RealWorkStandardInfo.dl_UseTool2.options.length;i++)
			{
				if(document.RealWorkStandardInfo.dl_UseTool2.options[i].innerText == row.getCellFromKey("ToolName2").getValue())
				{
					document.RealWorkStandardInfo.dl_UseTool2.options[i].selected=true;
					
				}
			}
			
			//사용치구2
			for(var i=0;i<document.RealWorkStandardInfo.dl_UseJig2.options.length;i++)
			{
				if(document.RealWorkStandardInfo.dl_UseJig2.options[i].innerText == row.getCellFromKey("JigName2").getValue())
				{
					document.RealWorkStandardInfo.dl_UseJig2.options[i].selected=true;
					
				}
			}
			
			//사용공구3
			for(var i=0;i<document.RealWorkStandardInfo.dl_UseTool3.options.length;i++)
			{
				if(document.RealWorkStandardInfo.dl_UseTool3.options[i].innerText == row.getCellFromKey("ToolName3").getValue())
				{
					document.RealWorkStandardInfo.dl_UseTool3.options[i].selected=true;
					
				}
			}
			
			//사용치구3
			for(var i=0;i<document.RealWorkStandardInfo.dl_UseJig3.options.length;i++)
			{
				if(document.RealWorkStandardInfo.dl_UseJig3.options[i].innerText == row.getCellFromKey("JigName3").getValue())
				{
					document.RealWorkStandardInfo.dl_UseJig3.options[i].selected=true;
			...
			document.RealWorkStandardInfo.tb_PriorityOrder.value = row.getCellFromKey("PriorityOrder").getValue()	//우선순위
			document.RealWorkStandardInfo.hdProcess.value = row.getCellFromKey("ProcessSequenceNum").getValue() //공정순서
			document.RealWorkStandardInfo.lb_ProcessCode.value = row.getCellFromKey("ProcessCode").getValue() //공정코드
			
			
				
			document.RealWorkStandardInfo.tb_Cavity.value = row.getCellFromKey("Cavity").getValue();//Cavity
			document.RealWorkStandardInfo.tb_SetupTime.value = row.getCellFromKey("SetupTime").getValue()	//셋업타임
			document.RealWorkStandardInfo.tb_StandardTime.value = row.getCellFromKey("StandardTime").getValue();//표준시간
			document.RealWorkStandardInfo.tb_RealProcessingTime.value = row.getCellFromKey("RealProcessingTime").getValue()	//실가공시간
			document.RealWorkStandardInfo.tb_SpaceTime.value = row.getCellFromKey("SpaceTime").getValue();//여유시간
			document.RealWorkStandardInfo.tb_WaitTime.value = row.getCellFromKey("WaitTime").getValue();	//대기시간
			document.RealWorkStandardInfo.tb_LotSize.value = row.getCellFromKey("LotSize").getValue();//LotSize
			document.RealWorkStandardInfo.lb_Index.value = row.getCellFromKey("RealWorkStandardInfoIndex").getValue();
			
			
			document.RealWorkStandardInfo.bt_Update.disabled = false;
			document.RealWorkStandardInfo.bt_Delete.disabled = false;
			document.RealWorkStandardInfo.bt_Registration.disabled = true;
		}
		
		function Ultrawebgrid3_DblClickHandler(gridName, cellId){
			//Add code to handle your event here.
			var row = igtbl_getRowById(cellId);
			document.RealWorkStandardInfo.hdProcessNum.value = row.getCellFromKey("ProcessSequenceNum").getValue() //공정순서
			document.RealWorkStandardInfo.lb_ProcessCode.value = row.getCellFromKey("ProcessCode").getValue() //공정코드
			document.RealWorkStandardInfo.hdProcess.value = row.getCellFromKey("ProcessCode").getValue() //공정코드
		...
		function Ultrawebgrid2_AfterSelectChangeHandler(gridName, id){
			...
			RealWorkStandardInfo.lb_ReferenceTable.value = row.getCellFromKey("CommunityDataTableID").getValue();
			RealWorkStandardInfo.lb_ReferenceIndex.value = row.getCellFromKey("CDTIndex").getValue();
		}
		...
		function Ultrawebgrid3_MouseOverHandler(gridName, id, button){
            ...
            document.RealWorkStandardInfo.lb_RowIndex.value = parts[1];
            }
		}
...
--></SCRIPT>
	...
	<body bgColor="#f7f6f6" MS_POSITIONING="GridLayout">
		<form id="RealWorkStandardInfo" method="post" runat="server">
					...
					<TD width="800">
						<table cellSpacing="0" cellPadding="0" width="100%">
							<TR>
								...
								<TD vAlign="bottom" align="right" width="200"><asp:linkbutton id="LinkButton1" runat="server" Font-Size="9pt">진작업표준정보보기</asp:linkbutton></TD>
							</TR>
						</table>
...
<BorderDetails ColorTop="White" WidthLeft="1px" WidthTop="1px" ColorLeft="White">
								...
								<TR>
									...
									<TD align="left" width="130"><INPUT id="lb_Index" style="WIDTH: 50px" type="hidden" value="0" name="Hidden1" runat="server"></TD>
