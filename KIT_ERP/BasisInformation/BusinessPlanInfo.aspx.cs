	...
	public class BusinessPlanInfo : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				...
				tb_PlanQuantity.Attributes.Add("OnKeyUp", "Process();OnKeyUp_Currency(this);");
				...
				// onkeyup 이벤트가 발생하면 수량입력과동시에 금액을 계산하는 자바스크립트 함수 매핑
				tb_SaleUnitCost.Attributes.Add("OnKeyUp", "Process();OnKeyUp_Currency(this);");
		...
		private void bt_Registration_Click(object sender, System.EventArgs e)
		{
			...
			MasterInfoRecordRUD rud = new  MasterInfoRecordRUD("BusinessPlanInfo","Registration","BPI_MT",(int)PageName.BusinessPlanInfo,Session["ID"].ToString(),InputList,"낱개입력");
			rud.ActivateRUD();
...
//			tb_ItemName.Text = "";
			...
			View view = new View("", "BusinessPlanInfo");
		...
		private void bt_Update_Click(object sender, System.EventArgs e)
		{
			...
			MasterInfoRecordRUD rud = new MasterInfoRecordRUD(m_RequirePage, m_RequireAction, m_TableName, m_Idx, Session["ID"].ToString(), InputList,"낱개입력");
			rud.ActivateRUD();
		...
		{
			RegisterStartupScript("","<script language=javascript>window.open('./Popup/BusinessPlan.aspx','BusinessPlan'," + "'width=850" + "," + "height=500px');</script>");
		}
		...
		private void bt_BusinessPlanView_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				View view = new View(ItemSearchControl1.ItemNum.ToString(), "BusinessPlanInfo");
				UltraWebGrid1.DataSource = view.ItemView();
				UltraWebGrid1.DataBind();
			}
		}
...
//			string formID = "BusinessPlanInfo" ;
