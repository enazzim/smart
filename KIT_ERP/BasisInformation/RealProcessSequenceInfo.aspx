<%@ Page language="c#" Codebehind="RealProcessSequenceInfo.aspx.cs" AutoEventWireup="false" Inherits="KIT_ERP.BasisInformation.RealProcessSequenceInfo" %>
...
<HTML>
	<HEAD>
		<title>RealProcessSequenceNumInfo</title>
		...
		function UltraWebGrid1_DblClickHandler(gridName, cellId)
		{
			...
			//공정명
			for(var i=0;i<document.RealProcessSequenceNumInfo.dl_ProcessCode.options.length;i++)
			{
				if(document.RealProcessSequenceNumInfo.dl_ProcessCode.options[i].value == row.getCellFromKey("ProcessCode").getValue())
				{
					document.RealProcessSequenceNumInfo.dl_ProcessCode.options[i].selected=true;
			...
			//작업구분
			for(var i=0;i<document.RealProcessSequenceNumInfo.dl_WorkDistinction.options.length;i++)
			{
				if(document.RealProcessSequenceNumInfo.dl_WorkDistinction.options[i].value == row.getCellFromKey("WorkDistinction").getValue())
				{
					document.RealProcessSequenceNumInfo.dl_WorkDistinction.options[i].selected=true;
					
					if(document.RealProcessSequenceNumInfo.dl_WorkDistinction.options[i].innerText == "외주")
					{
						document.RealProcessSequenceNumInfo.dl_WCName.disabled = true;
						document.RealProcessSequenceNumInfo.dl_WCName.value = "";
					}
					else
						document.RealProcessSequenceNumInfo.dl_WCName.disabled = false;
				}
			...
			//WC명
			for(var i=0;i<document.RealProcessSequenceNumInfo.dl_WCName.options.length;i++)
			{
				if(document.RealProcessSequenceNumInfo.dl_WCName.options[i].value == row.getCellFromKey("WCName").getValue())
				{
					document.RealProcessSequenceNumInfo.dl_WCName.options[i].selected=true;
					document.RealProcessSequenceNumInfo.hdWCName.value = row.getCellFromKey("WCName").getValue();
				}
			}
			...
			document.RealProcessSequenceNumInfo.tb_ProcessSequanceNum.value = row.getCellFromKey("ProcessSequenceNum").getValue()	//공정순서
			document.RealProcessSequenceNumInfo.tb_OutsideOrderRate.value = row.getCellFromKey("OutsideOrderRate").getValue();//외주비율
			document.RealProcessSequenceNumInfo.tb_ProgressRate.value = row.getCellFromKey("ProgressRate").getValue();	//진척비율
			document.RealProcessSequenceNumInfo.tb_LeadTime.value = row.getCellFromKey("LeadTime").getValue();//리드타임
			
			document.RealProcessSequenceNumInfo.lb_Index.value = row.getCellFromKey("RealProcessSequenceInfoIndex").getValue();
			
			document.RealProcessSequenceNumInfo.bt_Update.disabled = false;
			document.RealProcessSequenceNumInfo.bt_Delete.disabled = false;
			document.RealProcessSequenceNumInfo.bt_Registration.disabled = true;
			
			if(row.getCellFromKey("WorkDistinction").getValue() == '자가') 
			{
				document.RealProcessSequenceNumInfo.tb_LeadTime.disabled = true;				//리드타임	비활성화
				document.RealProcessSequenceNumInfo.tb_OutsideOrderRate.disabled = true;		//발주비율	비활성화
			}
			else if(row.getCellFromKey("WorkDistinction").getValue() == '외주')
			{
				document.RealProcessSequenceNumInfo.tb_LeadTime.disabled = false;				//리드타임 활성화
				document.RealProcessSequenceNumInfo.tb_OutsideOrderRate.disabled = true;		//발주비율 비활성화
			}
			else
			{
				document.RealProcessSequenceNumInfo.tb_LeadTime.disabled = false;				//리드타임 활성화
				document.RealProcessSequenceNumInfo.tb_OutsideOrderRate.disabled = false;		//발주비율 활성화
			}
		...
		function Ultrawebgrid2_AfterSelectChangeHandler(gridName, id){
			...
			RealProcessSequenceNumInfo.lb_ReferenceTable.value = row.getCellFromKey("CommunityDataTableID").getValue();
			RealProcessSequenceNumInfo.lb_ReferenceIndex.value = row.getCellFromKey("CDTIndex").getValue();
		}
	...
	<body bgColor="#f7f6f6" MS_POSITIONING="GridLayout">
		<form id="RealProcessSequenceNumInfo" method="post" runat="server">
					...
					<TD vAlign="top">
						<table style="HEIGHT: 20px" cellSpacing="0" cellPadding="0" width="100%">
							<TR>
								...
								<td vAlign="bottom" align="right" width="200"><asp:linkbutton id="LinkButton1" runat="server" Font-Size="10pt">진공정순서보기</asp:linkbutton></td>
							</TR>
						</table>
...
<BorderDetails ColorTop="White" WidthLeft="1px" WidthTop="1px" ColorLeft="White">
											...
											<Bands>
												<igtbl:UltraGridBand>
													<Columns>
														...
														<igtbl:UltraGridColumn HeaderText="번호" Key="RealProcessSequenceInfoIndex" BaseColumnName="RealProcessSequenceInfoIndex">
															<CellStyle VerticalAlign="Middle" HorizontalAlign="Center"></CellStyle>
														</igtbl:UltraGridColumn>
													</Columns>
												</igtbl:UltraGridBand>
								...
								<TR>
									...
									<TD align="left" width="130">
										<asp:textbox id="tb_OutsideOrderRate" style="TEXT-ALIGN: right" runat="server" BackColor="#EEEEE9"
									...
									<TD align="right" width="70"><FONT face="굴림">기타&nbsp; </FONT>
									</TD>
									<TD align="left" width="130">
