...
{
	/// <summary>
	/// CopyRealBOM에 대한 요약 설명입니다.
	/// </summary>
	public class CopyRealBOM : System.Web.UI.Page
	{
		...
		private void CopyBom()
		{
			...
			try
			{
				string str = "Select Count(*) From RIOI_MT Where ParentItemNum = @ItemNum and RecodingState = 1";
		...
		private void InsertRIOI(SqlConnection con, SqlTransaction trans)
		{
			string str = @"Select ParentItemNum, ChildItemNum, NeedQuantityNumerator, NeedQuantityDenominator, ProcessManagement, SubDivision, SupplyDivision, BOMUnit, BeginDate, EndDate, RecodingState, RegistrationPerson, RegistrationPersonID, RegistrationDate From RIOI_MT Where ParentItemNum = @ItemNum and RecodingState =1";
			SqlCommand comm = new SqlCommand();
			comm.Connection = con;
			...
			{
				str = @"Insert into RIOI_MT (ParentItemNum, ChildItemNum, NeedQuantityNumerator, NeedQuantityDenominator, ProcessManagement, SubDivision, SupplyDivision, BOMUnit, BeginDate, EndDate, RecodingState, RegistrationPerson, RegistrationPersonID, RegistrationDate)
				...
				str = @"Insert into UPRIOI_HT (ParentItemNum, ChildItemNum, NeedQuantityNumerator, NeedQuantityDenominator, ProcessManagement, SubDivision, SupplyDivision, BOMUnit, BeginDate, EndDate, UpdateReason, UpdatingPerson, UpdatingPersonID, UpdatingDate,RealItemOrganizationInfoIndex)
							values (@ParentItemNum, @ChildItemNum, @NeedQuantityNumerator, @NeedQuantityDenominator, @ProcessManagement, @SubDivision, @SupplyDivision, @BOMUnit, @BeginDate, @EndDate, '초기등록', @UpdatingPerson, @UpdatingPersonID, @UpdatingDate,@RealItemOrganizationInfoIndex)";
				comm.CommandText = str;
				comm.Parameters.Add("@ParentItemNum",ItemSearch1.ChildItemNum);
				comm.Parameters.Add("@ChildItemNum",dr["ChildItemNum"].ToString());
				comm.Parameters.Add("@NeedQuantityNumerator", decimal.Parse(dr["NeedQuantityNumerator"].ToString()));
				...
				comm.Parameters.Add("@RealItemOrganizationInfoIndex",MaxRIOI_MT(con,trans,ItemSearch1.ChildItemNum,dr["ChildItemNum"].ToString()));
				comm.ExecuteNonQuery();
				comm.Parameters.Clear();
			}
		...
		/// <returns></returns>
		private int MaxRIOI_MT(SqlConnection con,SqlTransaction trans,string ParentItem, string ChildItem)
		{
