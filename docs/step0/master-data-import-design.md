...
자산분류: `원자재`/`제품`/`상품`/`공정품`/`부자재`/`팬텀`  
(`팬텀`은 일괄등록 전용. 기준정보 품목 화면에서는 선택 불가)  
검사구분: `NONE`/`INSPECTION`

### 4.3 품목구성

모품목번호, 자품목번호, 모품수량, 자품수량 (모두 필수; 모품수량 > 0, 자품수량 ≥ 0 — 0은 하위 전개 제외용)
...
품목번호, 순서번호, 공정코드, 작업구분(`INHOUSE`/`OUTSOURCE`/`SPLIT`), 작업장명(조건부), 발주비율(SPLIT 0~100; OUTSOURCE·INHOUSE는 무시→0), 진척비율(기본 100)
