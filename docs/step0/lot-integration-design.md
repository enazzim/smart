...
> **문서 버전:** 2.6  
> **작성일:** 2026-07-10  
> **개정:** 2026-07-13 — §8.2.1 실사 보정 기타입출고 체크리스트 · FE 안내  
...
### 8.2.1 실사 보정 — 기타입출고 체크리스트

**원칙:** Lot·슬롯 잔량은 **직접 UPDATE 금지**. 실사 차이·보정은 **기타 입출고(TX)** 로만 반영한다.  
(불변식: `Σ Lot 슬롯 잔량 = inventory_balance.stock_qty`)

| # | 확인 | 내용 |
|---|------|------|
| 1 | 품목 | `lot_tracked=1`인지 확인. OFF면 Lot UI·검증이 없다. |
| 2 | 공정 (공정품·제품) | **공정 선택 후** 미리보기(창고·현재고·Lot UI)가 뜬다. 미선택 시 Lot 입력란 미표시. |
| 3 | 차이 방향 | 실사 > 장부 → **입고(IN)** / 실사 < 장부 → **출고(OUT)**. 수량 = \|차이\|. |
| 4 | Lot 지정 | **OUT**: 잔량 있는 Lot 필수 선택. **IN**: 기존 Lot 선택 **또는** 신규 번호 **또는** 자동생성. |
| 5 | 사유 | 사유코드·내용에 `실사보정` 등 남겨 감사·원장 추적. |
| 6 | 검증 | 등록 후 **재고·원장** 슬롯 현재고와 **Lot 잔량** 합이 일치하는지 확인. |
| 7 | 잠금만 | 수량 변경 없이 사용 금지면 Lot 마스터에서 `BLOCKED` (잔량 유지). |
| 8 | 금지 | Lot 마스터/DB에서 `qty_on_hand` 직접 수정 · 잔량 있는 Lot 삭제 · 차이만큼 Lot 잔량만 덮어쓰기. |

**화면 안내:** 기타 입출고·Lot 마스터 헤더에 동일 원칙을 짧게 표시한다.
...
| 2.6 | 2026-07-13 | §8.2.1 실사 보정 기타입출고 체크리스트 · FE 안내 문구 |

---

*v2.6 · Git commit은 사용자 요청 시*
